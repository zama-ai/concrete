...
_yosis_exe = None  # pylint: disable=invalid-name


def detect_yosis_exe():
    """Detect yosis executable."""
    global _yosis_exe  # noqa: PLW0603 pylint: disable=global-statement
    if _yosis_exe:
        return _yosis_exe
    result = (
        shutil.which(YOSIS_EXE_NAME)
        or shutil.which(YOSIS_EXE_NAME, path=os.path.dirname(sys.executable))
        or shutil.which(YOSIS_EXE_NAME, path=os.path.dirname(shutil.which("python3") or ""))
    )
    if result is None:
        msg = f"{YOSIS_EXE_NAME} cannot be found."
...
def _yosys_run_script(
    ...
    yosys_file.flush()
    yosys_call = [detect_yosis_exe(), "-s", yosys_file.name]
    try:
...
class ValueNode:
    ...
    def parameter(cls, base_name, i):
        ...
        return ValueNode(
            f"{base_name}[{i}]", ValueOrigin(is_parameter=True, bit_index=i), base_name
        )
    ...
    def result(cls, base_name, i):
        ...
        return ValueNode(f"{base_name}[{i}]", ValueOrigin(is_result=True, bit_index=i), base_name)
...
def convert_yosys_json_to_circuit(json_data, circuit_name="noname"):
    ...
    log("Names:", symbolic_name)
    log("Nodes:", nodes)
    return Circuit(circuit_name, parameters, results, nodes)
