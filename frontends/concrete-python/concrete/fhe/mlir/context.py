...
class Context:
    ...
    def round_bit_pattern(
        ...
        if exactness is Exactness.APPROXIMATE:
            ...
            # son the target precision can be matched, this is only possible because of the extra-bit from overflow protection
            ...
            if (
                ...
                # The last half-cell values in overflow_precision will naturally overflow due to approximate mode.
                # But there can also be an off by minus 1 to the previous cell in the worst case but an overflow in the successor TLU.
                ...
                if x.is_signed:
                    ...
                    used_positive_size = half_tlu_size // 2 - 1
                    ...
                    prevent_overflow_positive = (
                        # pre-overflow
                        [0] * (used_positive_size - 1)
                        # overflow part
                        + [full_decrease_by // 2, 3 * full_decrease_by // 4, full_decrease_by]
                        # unused
                        + [0] * (positive_size - used_positive_size)
                    )[
                        :positive_size
                    ]  # we cut in case the 3 corrected cells are too much
                    print(prevent_overflow_positive)
                    prevent_overflow = prevent_overflow_positive + [0] * negative_size
                    print(prevent_overflow)
                else:
                    prevent_overflow = (
                        # pre-overflow
                        [0] * (half_tlu_size - 1)
                ...
                if approx_conf.reduce_precision_after_approximate_clipping:
                    ...
                    # The TLU after will be adjusted to the right precision if needed (see `Converter.tlu`)
                else:
    ...
    def reinterpret(self, x: Conversion, *, bit_width: int, signed: bool = None) -> Conversion:
