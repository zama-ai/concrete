...
class StaticKeyValueDatabase:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if compiled:
            self.module = self._module(configuration)
    ...
    def _module(self, configuration: fhe.Configuration) -> fhe.Module:
        ...
        return StaticKeyValueDatabaseModule.compile(
            {
                "reset": [sample_state],
...
def insert(db: StaticKeyValueDatabase, key: int, value: int):
    ...
    _, encrypted_key, encoded_value = db.insert.encrypt(None, encoded_key, encoded_value)

    print()

    print("Inserting...")
    start = time.time()
    db.state = db.insert.run(db.state, encrypted_key, encoded_value)
    end = time.time()
...
def replace(db: StaticKeyValueDatabase, key: int, value: int):
    encoded_key, encoded_value = db.encode_key(key), db.encode_value(value)
    _, encrypted_key, encoded_value = db.replace.encrypt(None, encoded_key, encoded_value)

    print()

    print("Replacing...")
    start = time.time()
    db.state = db.replace.run(db.state, encrypted_key, encoded_value)
    end = time.time()
    print(f"(took {end - start:.3f} seconds)")
...
def query(db: StaticKeyValueDatabase, key: int) -> Optional[int]:
    ...
    _, encrypted_key = db.query.encrypt(None, encoded_key)
    ...
    found, value = db.query.decrypt(encrypted_found, encrypted_value)
    if not found:
        return None

    return db.decode_value(value)
