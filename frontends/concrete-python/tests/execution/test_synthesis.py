...
def test_unsigned_add_correctness():
    ...
    assert False
    @fhe.circuit({"a": "encrypted", "b": "encrypted"})
    def circuit(a: ty_in, b: ty_in):
        return op(a=a, b=b)

    r = list(range(0, 2**ty_in.dtype.bit_width))
    for a in r:
        for b in r:
            # assert op(a=a, b=b) == (a + b) % 2**5
            # assert circuit.simulate(a, b) == (a + b) % 2**5
            expected = (a * b) % 2**ty_out.dtype.bit_width
            assert op(a=a, b=b) == expected
            assert circuit.simulate(a, b) == expected
    assert False

def test_unsigned_sum_correctness():
    """
    Check add.
    """
    ty_in = fhe.uint1
    ...
    import math
    expression = " + ".join(
        f"{2**int(math.log2(i // 3 + 1))}*{v}"
        for i, v in enumerate(params)
    )
    print(expression)
    op = synth.verilog_expression(params, expression, ty_out, name="titi")
...
def test_unsigned_mul_forced_to_sum():
    """
    Check add.
    """
    print("TEST")
    ...
    elements = sorted([
        (2**(i+j), i, j)
        for i in range(ty_in.dtype.bit_width)
        for j in range(ty_in.dtype.bit_width)
    ])
    params_a = {
        f"a_{i}": fhe.uint1
        for _, i, _ in elements
    }
    params_b = {
        f"b_{j}": fhe.uint1
        for _, _, j in elements
    }
    params = {
        **params_a,
        **params_b
...
if __name__ == "__main__":
    # test_unsigned_sum_correctness()
    test_unsigned_mul_forced_to_sum()