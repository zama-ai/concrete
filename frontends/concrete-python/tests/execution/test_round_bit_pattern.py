...
@pytest.mark.parametrize("signed,accumulator_precision",
    [(signed, accumulator_precision) for signed in (True, False) for accumulator_precision in (13, 32)]
)
def test_round_bit_pattern_approximate_acc_to_6_costs(signed, accumulator_precision, helpers):
    ...
    if signed:
        ...
        input_domain = range(0, 2 ** (accumulator_precision) - 1)
    ...
    approx_conf_safest = fhe.ApproximateRoundingConfig(approximate_clipping_start_precision=False)
    ...
    def function(x):
        ...
        x = fhe.round_bit_pattern(
            x, lsbs_to_remove=lsbs_to_remove, overflow_protection=False
        )  # REVIEW:need to force does not work only with inputset ???
        x = x // 2**lsbs_to_remove
        return x

    # without overflow
    if signed:
        input_domain = range(-(2 ** (accumulator_precision - 1)), 2 ** (accumulator_precision - 2))
    else:
        input_domain = range(0, 2 ** (accumulator_precision - 1))
