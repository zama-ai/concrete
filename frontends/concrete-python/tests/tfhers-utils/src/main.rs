...
fn safe_save<T: Serialize + Versionize + Named>(path: &String, value: &T) {
    ...
    println!("safe_save {}", path);
    // safe_deserialize len=131593
...
fn safe_load<T: DeserializeOwned + Unversionize + Named>(path: &String) -> T {
    ...
    // safe load content len=131392
    println!("safe load content len={} path={}",content.len(), path);
    let cursor = Cursor::new(content);
    // Called `Result::unwrap()` on an `Err` value: "io error: failed to fill whole buffer"
    safe_deserialize(cursor, SERIALIZE_SIZE_LIMIT).unwrap()
}
