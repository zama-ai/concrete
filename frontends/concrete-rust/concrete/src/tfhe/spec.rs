        ...
        impl ToTokens for ClientSpec {
            fn to_tokens(&self, tokens: &mut TokenStream) {
                let input_types_per_func = &self.input_types_per_func
                    .iter()
                    .map(|(key, value)| (key, value.iter().map(|maybe_type| match maybe_type {
                        Some(typ) => quote!{Some(#typ)},
                        None => quote!{ None },
                    })))
                    .map(|(key, value)| quote! { (#key.to_string(), vec![#(#value),*]) })
                    .collect::<Vec<_>>();
                let output_types_per_func = &self.output_types_per_func
                    .iter()
                    .map(|(key, value)| (key, value.iter().map(|maybe_type| match maybe_type {
                        Some(typ) => quote!{Some(#typ)},
                        None => quote!{ None },
                    ...
                    .map(|(key, value)| quote! { (#key.to_string(), vec![#(#value),*]) })
                    .collect::<Vec<_>>();
                let input_shapes_per_func = &self.input_shapes_per_func
                    .iter()
                    .map(|(key, value)| (key, value.iter().map(|maybe_shape| match maybe_shape {
                        Some(shape) => quote!{Some(vec![#(#shape),*])},
                        None => quote!{ None },
                    })))
                    .map(|(key, value)| quote! { (#key.to_string(), vec![#(#value),*]) })
                    .collect::<Vec<_>>();
                let output_shapes_per_func = &self.output_shapes_per_func
                    .iter()
                    .map(|(key, value)| (key, value.iter().map(|maybe_shape| match maybe_shape {
                        Some(shape) => quote!{Some(vec![#(#shape),*])},
                        None => quote!{ None },
