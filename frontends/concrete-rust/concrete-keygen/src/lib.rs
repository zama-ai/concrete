...
//! - `get_lwe_secret_key_from_client_keyset_wasm`: Retrieves an LWE secret key from a client keyset in a WebAssembly environment.
...
//! - `generate_keyset_wasm`: Generates a Concrete keyset in a WebAssembly environment.
//! - `generate_client_keyset_wasm`: Generates a Concrete client keyset in a WebAssembly environment.
//! - `get_client_keyset_message`: Extracts the client keyset from a keyset and returns it as a Cap'n Proto message builder.
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn get_lwe_secret_key_from_client_keyset_wasm(
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn generate_client_keyset_wasm(mut keyset_info_buffer: &[u8], secret_seed: u128) -> Vec<u8> {
    let reader =
        serialize::read_message_from_flat_slice(&mut keyset_info_buffer, ReaderOptions::new())
            .unwrap();
    let key_set_info = reader
...
/// # Returns
/// A serialized client keyset.
pub fn get_client_keyset(mut keyset_buffer: &[u8]) -> Vec<u8> {
    let reader =
        serialize::read_message_from_flat_slice(&mut keyset_buffer, ReaderOptions::new()).unwrap();
    let keyset = reader
        .get_root::<concrete_protocol_capnp::keyset::Reader>()
        .unwrap();
    let builder = get_client_keyset_message(keyset);
    serialize::write_message_to_words(&builder)
}
...
#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn get_client_keyset_wasm(keyset_buffer: &[u8]) -> Vec<u8> {
    get_client_keyset(keyset_buffer)
}
