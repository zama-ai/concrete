...
fn generate_server_function_invoke(
    ...
    let preludes = multizip((input_specs.iter(), args_idents.iter(), args_types.iter()))
        .map(|(spec, ident, typ)| {
            match spec{
                Some(_) => quote!{ <#typ as ::concrete::utils::into_value::IntoValue>::into_value(#ident).get_transport_value()},
                None => quote!{#ident}
            }
        });
