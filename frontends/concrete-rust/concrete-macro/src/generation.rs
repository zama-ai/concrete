...
fn generate_keyset(pi: &ProgramInfo) -> proc_macro2::TokenStream {
    ...
    let fields_idents = need_providing.iter().map(|(func_name, ith, ..)| {
        format_ident!("{func_name}_{ith}")
    }).collect::<Vec<_>>();

    let fields_types = need_providing.iter().map(|_| {
    ...
    let methods = need_providing.iter().map(|(func_name, ith, gi, ..)| {
        ...
        quote! {
            pub fn #method_name(mut self, key: &::tfhe::ClientKey) -> Self {
                self.#ident = Some(<::tfhe::ClientKey as concrete::tfhe::IntoLweSecretKey>::into_lwe_secret_key(key, Some(#kid)));
                self
            }
        }
    });
