...
#pragma omp parallel for default(none)                                         \
    shared(ahe_pk, n, compressionKey, lweCt, prod, params)                     \
...
mpz_class compressMPZCollapsed(mpz_vec &compressionKey,
                               hcs::djcs::public_key &ahe_pk,
                               std::vector<std::vector<uint64_t>> &cts,
                               const mpz_class &scale, LWEParams &params) {
  uint64_t n = params.n;
  mpz_vec prod(n + 1);

#pragma omp parallel for default(none)                                         \
    shared(n, cts, params, scale, ahe_pk, prod, compressionKey)
...
[[maybe_unused]] mpz_vec scale_pack(std::vector<std::vector<uint64_t>> &cts,
                                    const mpz_class &scale, LWEParams &params) {
  uint64_t n = params.n;
  mpz_vec ret(n + 1);
...
CompressedCiphertext compressBatchedScaleFirst(
  ...
  for (uint64_t i = 0; i < num_ahe_cts; i++) {
    ...
    std::vector<std::vector<uint64_t>> to_compress(cts.begin() + _start,
                                                   cts.begin() + _end);
    // assign the new ahe ct
    c_ct.ahe_cts[i] = compressMPZCollapsed(compressionKey, ahe_pk, to_compress,
                                           c_ct.scale, params);
    _start = _end;
    _end += c_ct.maxCts;
  }
...
CompressedCiphertext compressBatchedEncryptFirst(
  ...
  // construct every ahe ciphertext
#pragma omp parallel for default(none)                                         \
    shared(ahe_pk, num_ahe_cts, compressionKey, c_ct, cts, params)             \
    num_threads(num_ahe_cts) schedule(static)
  for (uint64_t i = 0; i < num_ahe_cts; i++) {
    // start by compressing the first ciphertext
    mpz_class p_ct =
        compressSingle(compressionKey, ahe_pk, cts[i * c_ct.maxCts], params);
    for (uint64_t j = 1; j < c_ct.maxCts; j++) {
      // check if we compressed everything, we break
      if (i * c_ct.maxCts + j >= cts.size())
        break;
      // scale the ahe ciphertext to leave room for the next lwe ct in the lower
      // bits
      ahe_pk.ep_mul(p_ct, p_ct, c_ct.scale);
      mpz_class curr = compressSingle(compressionKey, ahe_pk,
                                      cts[i * c_ct.maxCts + j], params);
