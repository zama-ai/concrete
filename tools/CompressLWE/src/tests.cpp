...
void test_batched_compress(uint64_t num_cts, uint64_t s) {
  ...
  CompressedCiphertext compressed_ct =
      compressBatched(key->compKey, key->ahe_pk, lwe_cts, params);
  ...
  mpz_vec decrypted = decryptCompressedBatched(compressed_ct, key->ahe_sk,
                                               params, num_cts, true);
...
//    }
//    std::cout << "<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<
//    std::endl; std::cout << "Scale-first Test Passed!" << std::endl; std::cout
//    << "S: " <<s<<" ; Ciphers:  "<< num_cts<< std::endl; std::cout << "Number
//    of Damgard CTs:  " << compressed_ct.ahe_cts.size()<< " ciphers"<<
//    std::endl; std::cout << "Time taken by compress: " <<
//    d_compress.count()/1000 << " ms" << std::endl;
//
//    std::cout << "Time taken by LWE key:  " << d_lwe_key.count() << "
//    microseconds" << std::endl; std::cout << "Time taken by LWE cts:  " <<
//    d_lwe_cts.count() << " microseconds" << std::endl; std::cout << "Time
//    taken by keygen:   " << d_keygen.count() << " microseconds" << std::endl;
//    std::cout << "Time taken by decrypt:  " << d_decrypt.count() << "
//    microseconds" << std::endl; std::cout <<
//    "<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<< std::endl;
//}
//
