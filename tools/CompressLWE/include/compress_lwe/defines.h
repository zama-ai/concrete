...
struct Keys {
  ...
  mpz_vec compKey;
  explicit Keys(size_t n = 630) { compKey = mpz_vec(n); }
};
...
struct LWEParams {
  ...
  LWEParams(uint64_t n, uint64_t log_q, uint64_t p, bool binaryKeys = false)
      : n(n), logQ(log_q), p(p), binaryKeys(binaryKeys) {
    qBig = 1_mpz << log_q;
  }
};
...
struct CompressedCiphertext {
  ...
  CompressedCiphertext(hcs::djcs::public_key &pk, LWEParams &lweParams)
      : ahe_pk(pk), lweParams(lweParams) {
    uint64_t bitwidth, s;
    s = ahe_pk.as_ptr()->s;
