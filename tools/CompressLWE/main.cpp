...
int main() {
  ...
  // 2- Compress a batch of LWE ciphertexts into 1 CompressedCiphertext [a
  // vector of paillier ciphers]
  CompressedCiphertext compressed_ct =
      compressBatched(keys.compKey, lwe_cts, params);

  // 3- Decrypt the batch of compressed ciphertexts
  std::vector<uint64_t> decrypted = decryptCompressedBatched(
      compressed_ct, keys.paiKeys.priv_key, params, num_cts);
