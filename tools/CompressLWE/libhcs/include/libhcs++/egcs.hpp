...
public:
  cipher() { c = egcs_init_cipher(); }

  ~cipher() { egcs_free_cipher(c); }

  egcs_cipher *as_ptr() const { return c; }

  void clear() { egcs_clear_cipher(c); }
...
public:
  ...
  ~public_key() { egcs_free_public_key(pk); }

  egcs_public_key *as_ptr() { return pk; }
  ...
  void clear() { egcs_clear_public_key(pk); }
};
...
public:
  ...
  ~private_key() { egcs_free_private_key(vk); }

  egcs_private_key *as_ptr() { return vk; }
  ...
  void clear() { egcs_clear_private_key(vk); }
};
