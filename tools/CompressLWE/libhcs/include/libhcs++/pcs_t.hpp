...
public:
  ...
  ~public_key() { pcs_t_free_public_key(pk); }

  pcs_t_public_key *as_ptr() { return pk; }

  std::shared_ptr<hcs::random> *get_rand() { return hr; }
  ...
  void clear() { pcs_t_clear_public_key(pk); }

  std::string export_json() { return std::string(pcs_t_export_public_key(pk)); }
...
public:
  ...
  ~private_key() { pcs_t_free_private_key(vk); }

  pcs_t_private_key *as_ptr() { return vk; }
  ...
  void clear() { pcs_t_clear_private_key(vk); }
...
public:
  ...
  ~polynomial() { pcs_t_free_polynomial(px); }

  pcs_t_polynomial *as_ptr() { return px; }
...
public:
  ...
  ~auth_server() { pcs_t_free_auth_server(au); }

  pcs_t_auth_server *as_ptr() { return au; }

  mpz_class share_decrypt(public_key &pk, mpz_class &cipher1) {
    mpz_class rop;
    pcs_t_share_decrypt(pk.as_ptr(), au, rop.get_mpz_t(), cipher1.get_mpz_t());
    return rop;
  }
...
inline void generate_key_pair(public_key &pk, private_key &vk,
                              const unsigned long bits, const unsigned long l,
