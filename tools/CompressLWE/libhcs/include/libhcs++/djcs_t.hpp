...
public:
  ...
  void clear() { djcs_t_clear_public_key(pk); }
...
public:
  ...
  ~private_key() { djcs_t_free_private_key(vk); }

  djcs_t_private_key *as_ptr() { return vk; }
  ...
  void clear() { djcs_t_clear_private_key(vk); }
...
inline void generate_key_pair(public_key &pk, private_key &vk,
  ...
  djcs_t_generate_key_pair(pk.as_ptr(), vk.as_ptr(), vk.get_rand(), bits, w, l);
}
...
public:
  polynomial(std::shared_ptr<hcs::random> &hr_) {
    pk = djcs_t_init_polynomial();
    hr = hr_;
  }

  ~public_key() { djcs_t_free_polynomial(pk); }

  djcs_t_poly *as_ptr() { return pk; }
...
public:
  ...
  ~auth_server() { djcs_t_free_auth_server(au); }

  djcs_t_auth_server *as_ptr() { return au; }

  share_decrypt(public_key &pk, mpz_class &rop, mpz_class &cipher1) {
    djcs_t_share_decrypt(pk.as_ptr(), au, rop.get_mpz_t(), cipher1.get_mpz_t());
  }
