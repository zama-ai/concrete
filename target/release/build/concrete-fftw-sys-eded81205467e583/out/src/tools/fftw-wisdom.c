...
"    writing wisdom to stdout (or to a file, using -o).\n"
 ...
 "       -n, --no-system-wisdom: don't read /etc/concrete-fftw/ system wisdom file\n"
	...
	user requests -T */
     ...
     while ((c = my_getopt(argc, argv, options)) != -1) {
	  switch (c) {
	      ...
	      case 'V':
		   printf("concrete-fftw-wisdom tool for FFTW version " VERSION ".\n");
		   printf(
"\n"
...
"Foundation, Inc., 51 Franklin Street, Fifth Floor, Boston, MA  02110-1301  USA\n"
	      ...
	      case 'w': {
		   ...
		   if (strcmp(my_optarg, "-") && !(w = fopen(my_optarg, "r"))) {
			fprintf(stderr,
				"concrete-fftw-wisdom: error opening \"%s\": ", my_optarg);
			perror("");
			exit(EXIT_FAILURE);
		   }
     ...
     if (system_wisdom)
	  if (!FFTW(import_system_wisdom)() && verbose)
	       fprintf(stderr, "concrete-fftw-wisdom: system-wisdom import failed\n");
     ...
     if (!output_fname)
	  ...
	  if (!(output_file = fopen(output_fname, "w"))) {
	       fprintf(stderr,
		       "concrete-fftw-wisdom: error creating \"%s\"", output_fname);
	       perror("");
	       exit(EXIT_FAILURE);
	  }
