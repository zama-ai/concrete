...
///
/// This trait is used by the types that build on the `Tensor` type to implement
/// multi-dimensional collections of various kind. In essence, this trait allows
/// to extract a tensor properly qualified to use all the methods of the
/// `Tensor` type: ```rust
...
/// A trait for [`Tensor`]-based types, allowing to mutably borrow the enclosed
/// tensor.
///
/// This trait implements the same logic as `AsRefTensor`, but for mutable
/// borrow instead. See the [`AsRefTensor`] documentation for more explanations
/// on the logic.
