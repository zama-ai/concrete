...
///
/// This csprng is used by every objects that needs sampling in the library. If
/// the proper instructions are available on the machine, it will use an
/// hardware-accelerated variant for the generation. If not, a fallback software
/// version will be used.
///
/// # Safe multithreaded use
///
/// When using a csprng in a multithreaded setting, it is important to make sure
/// that the same sequence of bytes is not generated twice on two different
/// threads. This csprng offers a simple way to ensure that: any generator can
/// be _forked_ into several _bounded_ generators, which are able to sample a
/// fixed number of bytes. This forking operation has the effect of shifting the
/// state of the parent generator accordingly. This way, the children generators
...
/// # Bounds checking
///
/// Every generator created with the `RandomGenerator::new` method is unbounded.
/// This means that it can generate as much bytes as needed, and that it can
/// fork into as many children as needed. On the other hand, every generator
/// created with the `RandomGenerator::try_fork` method is bounded. It can only
/// generate a fixed amount of bytes, and it can only be forked if the total
...
impl RandomGenerator {
    ...
    /// Returns the number of bytes that can still be generated, if the
    ...
    /// Tries to fork the current generator into `n_child` generator bounded to
    /// `bytes_per_child`. If `n_child*bytes_per_child` exceeds the bound of
    /// the current generator, the method returns `None`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Tries to fork the current generator into `n_child` generator bounded to
    /// `bytes_per_child`, as a parallel iterator.
    ///
    /// If `n_child*bytes_per_child` exceeds the bound of the current generator,
    ...
    /// Generates an unsigned integer whose n least significant bits are
    /// uniformly random, and the other bits are zero.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// Fills an `AsMutTensor` value with random values whose n lsbs are sampled
    /// uniformly.
    ///
    /// # Example
    ///
    ...
    /// Generates a tensor of random uniform values, whose n lsbs are sampled
    /// uniformly.
    ///
    /// # Example
    ///
    ...
    /// Generates an unsigned integer whose n most significant bits are
    /// uniformly random, and the other bits are zero.
    ///
    /// # Example
    ///
    ...
    /// Generates a tensor of random uniform values, whose n msbs are sampled
    /// uniformly.
    ///
    /// # Example
    ///
    ...
    /// Generates a random uniform unsigned integer with probability
    /// `1-prob_zero`, and a zero value with probability `prob_zero`.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// Generates a tensor of a given size, whose coefficients are random
    /// uniform with probability `1-prob_zero`, and zero with probability
    /// `prob_zero`.
    ///
    /// # Example
    ///
    ...
    /// Generates two floating point values drawn from a gaussian distribution
    /// with input mean and standard deviation.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// Generates a new tensor of floating point values, randomly sampled from a
