...
func.func @lsb(%arg0: !FHE.eint<7>) -> !FHE.eint<7> {
  ...
  // CHECK-NEXT:  %cst = arith.constant dense<-36028797018963968> : tensor<256xi64>
  ...
  // CHECK-NEXT:  %3 = "TFHE.bootstrap_glwe"(%2, %cst) {key = #TFHE.bsk<sk?, sk?, -1, -1, -1, -1>} : (!TFHE.glwe<sk?>, tensor<256xi64>) -> !TFHE.glwe<sk?>
