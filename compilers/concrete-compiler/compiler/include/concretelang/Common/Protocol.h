...
template <typename MessageType> struct Message {
  ...
  Result<void> readBinaryFromString(const std::string &input) {
    auto istream = std::istringstream(input);
    return this->readBinaryFromIstream(istream);
  }
  ...
  std::string __attribute__((noinline, used)) debugString() const {
