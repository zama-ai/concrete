...
#ifdef OUTPUT_COMPRESSION_SUPPORT
  std::optional<std::shared_ptr<PaiCiphertext>> compressionKey;
#endif
...
public:
  ...
  EvaluationKeys(
      const std::vector<LweKeyswitchKey> keyswitchKeys,
      const std::vector<LweBootstrapKey> bootstrapKeys,
      const std::vector<PackingKeyswitchKey> packingKeyswitchKeys
#ifdef OUTPUT_COMPRESSION_SUPPORT
      ,
      const std::optional<std::shared_ptr<PaiCiphertext>> compressionKey
#endif
      )
...
#ifdef OUTPUT_COMPRESSION_SUPPORT
  const std::optional<std::shared_ptr<PaiCiphertext>>
  getCompressionKey() const {
