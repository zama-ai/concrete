...
KeySetCache::loadKeys(ClientParameters &params, uint64_t secret_seed_msb,
  ...
  OUTCOME_TRY(auto keySet,
              KeySet::fromKeys(params, secretKeys, bootstrapKeys, keyswitchKeys,
                               packingKeyswitchKeys, std::move(secretCsprng),
                               std::move(encryptionCsprng)));
...
outcome::checked<std::unique_ptr<KeySet>, StringError>
KeySetCache::loadOrGenerateSave(ClientParameters &params,
  ...
  llvm::sys::path::append(folderPath,
                          std::to_string(secret_seed_msb) + "_" +
                              std::to_string(secret_seed_lsb) + "_" +
                              std::to_string(encryption_seed_msb) + "_" +
  ...
  if (llvm::sys::fs::exists(folderPath)) {
    ...
    auto keys =
        loadKeys(params, secret_seed_msb, secret_seed_lsb, encryption_seed_msb,
                 encryption_seed_lsb, std::string(folderPath));
    if (keys.has_value()) {
      return keys;
    } else {
  ...
  OUTCOME_TRY(auto key_set, KeySet::generate(params, std::move(secretCsprng),
                                             std::move(encryptionCsprng)));
...
KeySetCache::generate(std::shared_ptr<KeySetCache> cache,
                      ClientParameters &params, uint64_t secret_seed_msb,
                      uint64_t secret_seed_lsb, uint64_t encryption_seed_msb,
                      uint64_t encryption_seed_lsb) {
#ifdef CONCRETELANG_GENERATE_UNSECURE_SECRET_KEYS
  getApproval();
  ...
  return cache ? cache->loadOrGenerateSave(params, secret_seed_msb,
                                           secret_seed_lsb, encryption_seed_msb,
                                           encryption_seed_lsb)
               : KeySet::generate(params, std::move(secretCsprng),
                                  std::move(encryptionCsprng));
}

outcome::checked<std::unique_ptr<KeySet>, StringError>
KeySetCache::generate(ClientParameters &params, uint64_t secret_seed_msb,
                      uint64_t secret_seed_lsb, uint64_t encryption_seed_msb,
                      uint64_t encryption_seed_lsb) {
#ifdef CONCRETELANG_GENERATE_UNSECURE_SECRET_KEYS
  getApproval();
  ...
  return loadOrGenerateSave(params, secret_seed_msb, secret_seed_lsb,
