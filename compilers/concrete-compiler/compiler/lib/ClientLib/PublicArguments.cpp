...
#ifdef OUTPUT_COMPRESSION_SUPPORT
      assert(std::holds_alternative<std::shared_ptr<CompressedCiphertext>>(
          a.get()));
#endif
      exit(1);
    }
...
decrypt_no_decode_one_result(const KeySet &keySet, uint pos, CircuitGate gate,
  ...
  if (std::holds_alternative<TensorData>(buffers)) {
    ...
    assert(
        std::holds_alternative<std::shared_ptr<CompressedCiphertext>>(buffers));

    auto &arg = std::get<std::shared_ptr<CompressedCiphertext>>(buffers);

    LWEParams params(lweSize - 1, 64);

    auto &paiFullKeys = keySet.getPaiFullKey();

    assert(paiFullKeys.has_value());

    auto &privKey = paiFullKeys->priv_key;

    decrypted_vector =
        decryptCompressedBatched(*arg, *privKey, params, number_lwe_to_decrypt);
#else
    exit(1);
#endif
  }
