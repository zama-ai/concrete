...
#ifdef OUTPUT_COMPRESSION_SUPPORT
  keySet->paiFullKeys = std::move(paiFullKeys);
#endif
  OUTCOME_TRYV(keySet->setupEncryptionMaterial());
  return std::move(keySet);
}
...
EvaluationKeys KeySet::evaluationKeys() {
  ...
  std::optional<std::shared_ptr<PaiCiphertext>> compressionKeys;

  if (paiFullKeys.has_value()) {
    compressionKeys = paiFullKeys->compKey;
  }
#endif
...
#ifdef OUTPUT_COMPRESSION_SUPPORT
      PaiFullKeys paifullkey;

      if (gate.compression) {
        output = {gate, skIt, std::move(paifullkey)};
      } else {
...
outcome::checked<void, StringError> KeySet::generatePaiKeys(PaiKeyParam param) {
  ...
  paiFullKeys = generateKeys(sk.vector());
...
#ifdef OUTPUT_COMPRESSION_SUPPORT
const std::optional<PaiFullKeys> &KeySet::getPaiFullKey() const {
  return paiFullKeys;
}
#endif
