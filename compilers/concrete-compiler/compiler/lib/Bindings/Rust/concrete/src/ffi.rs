...
mod ffi {
    unsafe extern "C++" {
        ...
        fn get_info_string(self: &Library) -> String;
        ...
        fn get_info_string(self: &LweBootstrapKey) -> String;

        type LweKeyswitchKey;
        fn get_buffer(self: Pin<&mut LweKeyswitchKey>) -> &[u64];
        fn get_info_string(self: &LweKeyswitchKey) -> String;

        type PackingKeyswitchKey;
        fn get_buffer(self: Pin<&mut PackingKeyswitchKey>) -> &[u64];
        fn get_info_string(self: &PackingKeyswitchKey) -> String;

        type LweSecretKey;
        fn get_buffer(self: Pin<&mut LweSecretKey>) -> &[u64];
        fn get_info_string(self: &LweSecretKey) -> String;

        type ClientKeyset;
    }
}

impl GetInfo for Library{
    type Output = ProgramInfo;

    fn get_info(&self) -> Self::Output {
        serde_json::from_str(&self.get_info_string()).unwrap()
    }
}

impl GetInfo for LweSecretKey {
    type Output = LweSecretKeyInfo;
    fn get_info(&self) -> Self::Output {
        serde_json::from_str(&self.get_info_string()).unwrap()
    }
}

impl GetInfo for LweKeyswitchKey {
    type Output = LweKeyswitchKeyInfo;
    fn get_info(&self) -> Self::Output {
        serde_json::from_str(&self.get_info_string()).unwrap()
    }
}

impl GetInfo for PackingKeyswitchKey {
    type Output = PackingKeyswitchKeyInfo;
    fn get_info(&self) -> Self::Output {
        serde_json::from_str(&self.get_info_string()).unwrap()
    }
}

impl GetInfo for LweBootstrapKey {
    type Output = LweBootstrapKeyInfo;
    fn get_info(&self) -> Self::Output {
        serde_json::from_str(&self.get_info_string()).unwrap()
    }
}
...
use crate::{protocol::{LweBootstrapKeyInfo, LweKeyswitchKeyInfo, LweSecretKeyInfo, PackingKeyswitchKeyInfo, ProgramInfo}, utils::GetInfo};
