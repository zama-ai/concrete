...
use crate::{pbs_p_fail_from_global_p_fail, MyRange, STEP};
...
impl Problem for LMPConstraint {
    ...
    fn verify(&self, param: Self::Param) -> bool {
        ...
        if UNDERESTIMATION {
            v_pbs * (1 << (2 * self.log_norm2) + 1) as f64 + v_ks + v_ms < self.variance_constraint
        } else {
        ...
        self.range_base_log_ks
            ...
            .flat_map(move |base_log_ks| {
                self.range_level_ks
                    ...
                    .into_iter()
                    .flat_map(move |level_ks| {
                        self.range_base_log_pbs.to_std_range().into_iter().flat_map(
                            move |base_log_pbs| {
                                self.range_level_pbs
                                    .to_std_range_tight(base_log_pbs, precision)
                                    .into_iter()
                                    .flat_map(move |level_pbs| {
                                        self.range_glwe_dim.to_std_range().into_iter().flat_map(
                                            move |glwe_dim| {
                                                self.range_log_poly_size
                                                    .to_std_range()
                                                    .into_iter()
...
pub fn solve_all_lmp(p_fail: f64, mut writer: impl Write) {
    let start = Instant::now();
    let precisions = 1..24;
    let log_norms = vec![4, 6, 8, 10];
    let p_fail_per_pbs = pbs_p_fail_from_global_p_fail(2, p_fail);

    let res: Vec<(u64, u64, Option<(LMPParams, f64)>)> = precisions
        .into_par_iter()
        .flat_map(|precision| {
            log_norms
                .clone()
...
pub fn write_to_file(
    ...
    res: &[(u64, u64, Option<(LMPParams, f64)>)],
) -> Result<(), std::io::Error> {
    writeln!(
        writer,
