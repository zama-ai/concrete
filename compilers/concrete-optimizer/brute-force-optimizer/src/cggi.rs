...
use crate::{ExplicitRange, MyRange, STEP};
...
impl CGGISearchSpaceTighten {
    ...
    fn par_iter(self) -> impl rayon::iter::ParallelIterator<Item=CGGIParams> {
        self.range_glwe_dim
            .to_std_range()
            .into_par_iter().map(|k| CGGIParams {
            base_log_ks: 0,
            level_ks: 0,
            base_log_pbs: 0,
            level_pbs: 0,
    ...
    fn iter(self, precision: u64) -> impl Iterator<Item = CGGIParams> {
                                ...
                                self.range_log_poly_size
                                    ...
                                    .flat_map(move |log_poly_size| {
                                        self.range_small_lwe_dim
                                            ...
                                            .step_by(STEP)
                                            .into_iter()
                                            .map(move |small_lwe_dim| CGGIParams {
                                                base_log_ks,
                                                level_ks,
                                                base_log_pbs,
                                                level_pbs,
...
pub fn solve_all_cggi(p_fail: f64, mut writer: impl Write) {
    ...
    let res: Vec<(u64, u64, Option<(CGGIParams, f64)>)> = precisions
        .into_par_iter()
        .flat_map(|precision| {
            log_norms
                .clone()
...
pub fn write_to_file(
    ...
    res: &[(u64, u64, Option<(CGGIParams, f64)>)],
) -> Result<(), std::io::Error> {
    writeln!(
        writer,
