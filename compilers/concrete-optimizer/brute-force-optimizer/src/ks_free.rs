...
impl KSFreeSearchSpace {
    ...
    fn par_iter(self) -> impl rayon::iter::ParallelIterator<Item=KSFreeParams> {
        self.range_glwe_dim
            .to_std_range()
            .into_par_iter().map(|k| KSFreeParams {
            base_log_pbs: 0,
            level_pbs: 0,
            glwe_dim: 0,
            log_poly_size: 0,
    ...
    fn iter(self, precision: u64) -> impl Iterator<Item = KSFreeParams> {
                    ...
                    .flat_map(move |level_pbs| {
                        self.range_glwe_dim
                            ...
                            .flat_map(move |glwe_dim| {
                                self.range_log_poly_size.to_std_range().into_iter().map(
                                    move |log_poly_size| KSFreeParams {
                                        base_log_pbs,
                                        level_pbs,
                                        glwe_dim,
                                        log_poly_size,
...
pub fn solve_all_ksfree(p_fail: f64, mut writer: impl Write) {
    let precisions = 1..9;
    let log_norms = vec![4, 6, 8, 10];

    let res: Vec<(u64, u64, Option<(KSFreeParams, f64)>)> = precisions
        .into_par_iter()
        .flat_map(|precision| {
            log_norms
                .clone()
...
pub fn write_to_file(
    ...
    res: &[(u64, u64, Option<(KSFreeParams, f64)>)],
) -> Result<(), std::io::Error> {
