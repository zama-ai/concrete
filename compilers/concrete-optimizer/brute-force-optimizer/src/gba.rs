...
pub fn solve_all_gba(p_fail: f64, mut writer: impl Write) {
    let nb_inputs = 2;
    ...
    let res: Vec<(u64, u64, Option<(GBAParams, f64)>)> = precisions
        .into_par_iter()
        .flat_map(|precision| {
            log_norms
                .clone()
    ...
    write_to_file(writer, &res, nb_inputs);
}
...
pub fn write_to_file(
    ...
    res: &[(u64, u64, Option<(GBAParams, f64)>)],
    nb_inputs: u64,
) -> Result<(), std::io::Error> {
    writeln!(
        writer,
