...
impl CJPSearchSpaceTighten {
    ...
    fn par_iter(self) -> impl rayon::iter::ParallelIterator<Item=MultiBitCJPParams> {
        self.range_glwe_dim
            .to_std_range()
            .into_par_iter().map(|k| MultiBitCJPParams {
            base_log_ks: 0,
            level_ks: 0,
            base_log_pbs: 0,
            level_pbs: 0,
                                ...
                                self.range_log_poly_size
                                    ...
                                    .flat_map(move |log_poly_size| {
                                        self.range_small_lwe_dim
                                            .to_std_range()
                                            .step_by(GROUPING_FACTOR as usize) // STEP)
                                            .into_iter()
...
pub fn solve_all_multi_bit_cjp(p_fail: f64, mut writer: impl Write) {
    ...
    let res: Vec<(u64, u64, Option<(MultiBitCJPParams, f64)>)> = precisions
        .into_par_iter()
        .flat_map(|precision| {
            log_norms
                .clone()
