...
use super::{
    ...
    symbolic::{fks, ks, Symbol, SymbolMap},
};
...
impl ComplexityExpression {
    ...
    pub fn evaluate_ks_max_cost(
        ...
        let actual_ks_cost = costs.0.get(ks(src_partition, dst_partition));
        let ks_coeff = self.0.get(ks(src_partition, dst_partition));
    ...
    pub fn evaluate_fks_max_cost(
        ...
        let actual_fks_cost = costs.0.get(fks(src_partition, dst_partition));
        let fks_coeff = self.0.get(fks(src_partition, dst_partition));
        let actual_complexity =
