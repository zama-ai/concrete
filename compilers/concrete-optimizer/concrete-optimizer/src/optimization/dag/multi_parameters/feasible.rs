...
use super::noise_expression::{s2_fks, s2_ks, s2_pbs, NoiseValues};
...
impl Feasible {
    ...
    pub fn pbs_max_feasible_variance(
        ...
        let actual_pbs_variance = operations_variance.variance(s2_pbs(partition));
        let mut smallest_pbs_max_variance = f64::MAX;

        for constraint in &self.undominated_constraints {
            let pbs_coeff = constraint.noise_expression.coeff(s2_pbs(partition));
            if pbs_coeff == 0.0 {
                continue;
            }
    ...
    pub fn ks_max_feasible_variance(
        ...
        let actual_ks_variance = operations_variance.variance(s2_ks(src_partition, dst_partition));
    ...
    pub fn fks_max_feasible_variance(
        ...
        let actual_fks_variance =
            operations_variance.variance(s2_fks(src_partition, dst_partition));
    ...
    pub fn filter_constraints(&self, partition: PartitionIndex) -> Self {
        ...
        let touch_any_ks = |constraint: &VarianceConstraint, i| {
            ...
            variance.coeff(s2_ks(partition, i)) > 0.0
                || variance.coeff(s2_ks(i, partition)) > 0.0
                || variance.coeff(s2_fks(partition, i)) > 0.0
                || variance.coeff(s2_fks(i, partition)) > 0.0
        };
