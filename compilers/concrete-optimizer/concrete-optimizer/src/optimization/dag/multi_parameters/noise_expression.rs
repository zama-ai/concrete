...
mod test {
    ...
    fn test_noise_expression() {
        let mut expr = 1. * s2_inp(PartitionIndex(0))
            + s2_pbs(PartitionIndex(0)) * 2.0
            + 5. * (3. * s2_inp(PartitionIndex(1)) + s2_pbs(PartitionIndex(1)) * 4.0);
        assert_eq!(expr.coeff(s2_inp(PartitionIndex(0))), 1.0);
        assert_eq!(expr.coeff(s2_pbs(PartitionIndex(0))), 2.0);
        assert_eq!(expr.coeff(s2_inp(PartitionIndex(1))), 15.0);
        assert_eq!(expr.coeff(s2_pbs(PartitionIndex(1))), 20.0);
        expr *= 4.;
        println!("{expr}");
        assert_eq!(expr.coeff(s2_inp(PartitionIndex(0))), 4.0);
        assert_eq!(expr.coeff(s2_pbs(PartitionIndex(0))), 8.0);
        assert_eq!(expr.coeff(s2_inp(PartitionIndex(1))), 60.0);
        assert_eq!(expr.coeff(s2_pbs(PartitionIndex(1))), 80.0);
        expr *= 0.;
