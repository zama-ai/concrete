...
use super::noise_expression::{s2_fks, s2_inp, s2_ks, s2_ms, s2_pbs, NoiseValues};
use super::symbolic::{fks, ks, pbs};
...
fn optimize_1_ks(
    ...
    for &ks_quantity in ks_pareto {
        ...
        if ks_variance <= ks_max_variance {
            operations
                .variance
                .set_variance(s2_ks(ks_src, ks_dst), ks_variance);
            operations.cost.set_cost(ks(ks_src, ks_dst), ks_cost);
...
fn optimize_1_fks_and_all_compatible_ks(
    ...
    for &ks_quantity in &ks_pareto {
        ...
        operations
            .cost
            .set_cost(fks(fks_src, fks_dst), fks_quantity.complexity);
        operations
            .variance
            .set_variance(s2_fks(fks_src, fks_dst), fks_quantity.noise);
...
fn optimize_1_cmux_and_dst_exclusive_fks_subset_and_all_ks(
    ...
    for &cmux_quantity in cmux_pareto {
        ...
        operations.cost.set_cost(pbs(partition), pbs_cost);
        ...
        operations
            .variance
            .set_variance(s2_pbs(partition), pbs_variance);
...
fn apply_fks_variance_and_cost_or_lower_bound(
    ...
    for (src, dst) in cross_partition(nb_partitions) {
        ...
        if input_glwe == output_glwe {
            operations.variance.set_variance(s2_fks(src, dst), 0.0);
            operations.cost.set_cost(fks(src, dst), 0.0);
            continue;
        }
        ...
        if let Some(this_fks) = initial_fks[src.0][dst.0] {
            ...
            if !to_be_optimized {
                if input_glwe == &this_fks.src_glwe_param && output_glwe == &this_fks.dst_glwe_param
                {
                    operations
                        .variance
                        .set_variance(s2_fks(src, dst), this_fks.noise);
                    operations.cost.set_cost(fks(src, dst), this_fks.complexity);
                }
                continue;
            }
        }
        ...
        operations.cost.set_cost(fks(src, dst), cost);
        ...
        operations
            .variance
            .set_variance(s2_fks(src, dst), variance_min);
    }
}
...
fn apply_partitions_input_and_modulus_variance_and_cost(
    ...
    for i in PartitionIndex::range(0, nb_partitions) {
        ...
        operations.variance.set_variance(s2_inp(i), input_variance);
        operations
            .variance
            .set_variance(s2_ms(i), variance_modulus_switching);
    }
}
...
fn apply_pbs_variance_and_cost_or_lower_bounds(
    ...
    for (i, this_pbs) in initial_pbs.iter().enumerate() {
        ...
        if let Some(this_pbs) = this_pbs {
            ...
            operations
                .variance
                .set_variance(s2_pbs(i), this_pbs.noise_br(internal_dim));
            operations
                .cost
                .set_cost(pbs(i), this_pbs.complexity_br(internal_dim));
        } else {
            ...
            operations.variance.set_variance(s2_pbs(i), variance_min);
            operations.cost.set_cost(pbs(i), 0.0);
        }
    }
}
...
fn used_tlu_keyswitch(dag: &AnalyzedDag) -> Vec<Vec<bool>> {
    ...
    for (src_partition, dst_partition) in cross_partition(dag.nb_partitions) {
        for constraint in &dag.variance_constraints {
            if constraint
                .noise_expression
                .coeff(s2_ks(src_partition, dst_partition))
                != 0.0
            {
...
fn used_conversion_keyswitch(dag: &AnalyzedDag) -> Vec<Vec<bool>> {
    ...
    for (src_partition, dst_partition) in cross_partition(dag.nb_partitions) {
        for constraint in &dag.variance_constraints {
            if constraint
                .noise_expression
                .coeff(s2_fks(src_partition, dst_partition))
                != 0.0
            {
                result[src_partition.0][dst_partition.0] = true;
                break;
...
fn sanity_check(
    ...
    for partition in PartitionIndex::range(0, nb_partitions) {
        ...
        operations
            .variance
            .set_variance(s2_inp(partition), input_variance);
        operations
            .variance
            .set_variance(s2_ms(partition), variance_modulus_switching);
        if let Some(this_pbs) = micro_params.pbs[partition.0] {
            operations
                .variance
                .set_variance(s2_pbs(partition), this_pbs.noise_br(internal_dim));
            operations
                .cost
                .set_cost(pbs(partition), this_pbs.complexity_br(internal_dim));
        } else {
            operations
                .variance
                .set_variance(s2_pbs(partition), f64::MAX);
            operations.cost.set_cost(pbs(partition), f64::MAX);
        }
        for src_partition in PartitionIndex::range(0, nb_partitions) {
            ...
            if let Some(this_ks) = micro_params.ks[src_partition.0][partition.0] {
                ...
                operations
                    .variance
                    .set_variance(s2_ks(src_partition, partition), this_ks.noise(src_lwe_dim));
                operations.cost.set_cost(
                    ks(src_partition, partition),
                ...
                operations
                    .variance
                    .set_variance(s2_ks(src_partition, partition), f64::MAX);
                operations
                    .cost
                    .set_cost(ks(src_partition, partition), f64::MAX);
            }
            if let Some(this_fks) = micro_params.fks[src_partition.0][partition.0] {
                ...
                operations
                    .variance
                    .set_variance(s2_fks(src_partition, partition), this_fks.noise);
                operations
                    .cost
                    .set_cost(fks(src_partition, partition), this_fks.complexity);
            } else {
                assert!(
                    !used_conversion_keyswitch[src_partition.0][partition.0],
                ...
                operations
                    .variance
                    .set_variance(s2_fks(src_partition, partition), f64::MAX);
                operations
                    .cost
                    .set_cost(fks(src_partition, partition), f64::MAX);
            }
        }
    }
    #[allow(clippy::float_cmp)]
