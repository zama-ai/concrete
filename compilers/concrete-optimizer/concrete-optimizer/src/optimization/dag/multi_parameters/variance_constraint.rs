...
impl VarianceConstraint {
    ...
    fn dominate_or_equal(&self, other: &Self) -> bool {
        ...
        for partition in PartitionIndex::range(0, self.nb_partitions) {
            let diffs = [
                rel_diff(&|expr| expr.coeff(s2_pbs(partition))),
                rel_diff(&|expr| expr.coeff(s2_pbs(partition)) + expr.coeff(s2_inp(partition))),
                rel_diff(&|expr| expr.coeff(s2_ms(partition))),
            ];
            for diff in diffs {
                if diff < 0.0 {
                    return false;
        ...
        for src_partition in PartitionIndex::range(0, self.nb_partitions) {
            for dst_partition in PartitionIndex::range(0, self.nb_partitions) {
                let diffs = [
                    rel_diff(&|expr| expr.coeff(s2_ks(src_partition, dst_partition))),
                    rel_diff(&|expr| expr.coeff(s2_fks(src_partition, dst_partition))),
                ];
                for diff in diffs {
                    if diff < 0.0 {
                        return false;
