...
use super::keys_spec;
use super::noise_expression::{s2_fks, s2_inp, s2_ks, s2_ms, s2_pbs, NoiseExpression};
use super::partitions::Partitions;
use super::symbolic::{fks, ks, pbs, SymbolMap};
        ...
        let vars = self
            ...
            .map(|op| match op.operator {
                Operator::Input { .. } => {
                    ...
                    output[op_partition] += 1.0 * s2_inp(op_partition);
                    output
                }
            ...
            if let Operator::Input { .. } = op {
                ...
                if p_cut.is_external_partition(&partition_index) {
                    ...
                    variances.vars[partition_index.0] = NoiseExpression::zero()
                        + (max_variance / variance) * s2_pbs(partition_index);
    ...
    fn forward_noise(&mut self) -> bool {
        ...
        for operator_id in self.dag.get_indices_iter() {
            ...
            operator_variance[operator.partition().instruction_partition] = match operator
                ...
                .operator
            {
                Operator::Lut { .. } => NoiseExpression::zero() + 1.0 * s2_pbs(operator_partition),
                Operator::Max { .. } => {
                    operator
                        .get_inputs_iter()
                ...
                Operator::Input { .. } | Operator::Zero { .. } => unreachable!(),
            ...
            operator
                ...
                .for_each(|index| {
                    ...
                    operator_variance[*index] =
                        noise_in_operator_partition + 1.0 * s2_fks(operator_partition, *index);
                });
        ...
        self.vars
            ...
            .flat_map(|(var_i, var)| {
                PartitionIndex::range(0, self.nb_partitions())
                    .map(move |part_i| (var_i, part_i, var.coeff(s2_inp(part_i))))
            })
        ...
        if let Operator::Lut { input, .. } = op.operator {
            ...
            let src_partition = match partitions[op.id].inputs_transition[0] {
                ...
                Some(Transition::Additional { src_partition }) => {
                    ...
                    assert!(variance.coeff(s2_fks(src_partition, dst_partition)) == 1.0);
                    dst_partition
                }
            };
            let variance = variances[*input][src_partition].clone();
            let variance =
                variance + 1.0 * s2_ks(src_partition, dst_partition) + 1.0 * s2_ms(partition);
...
fn operations_counts(
    ...
    if let Operator::Lut { input, .. } = op {
        ...
        counts.update(ks(src_partition, partition), |a| a + nb_lut);
        counts.update(pbs(partition), |a| a + nb_lut);
        for &conv_partition in &instr_partition.alternative_output_representation {
            counts.update(fks(partition, conv_partition), |a| a + nb_lut);
        }
    }
...
pub mod tests {
    ...
    fn assert_input_on(
        ...
        for symbolic_variance_partition in [LOW_PRECISION_PARTITION, HIGH_PRECISION_PARTITION] {
            ...
            let coeff = sb.coeff(s2_inp(symbolic_variance_partition));
    ...
    fn assert_pbs_on(
        ...
        for symbolic_variance_partition in [LOW_PRECISION_PARTITION, HIGH_PRECISION_PARTITION] {
            ...
            let coeff = sb.coeff(s2_pbs(symbolic_variance_partition));
            if symbolic_variance_partition == partition {
                assert!(
                    coeff == expected_coeff,
    ...
    fn test_rounded_v3_first_layer_and_second_layer() {
        ...
        for op_i in input1.0..lut1.0 {
            ...
            assert!(sb.coeff(s2_inp(p)) >= 1.0 || sb.coeff(s2_pbs(p)) >= 1.0);
        }
        ...
        assert!(sb.coeff(s2_inp(p)) == 0.0);
        assert!(sb.coeff(s2_pbs(p)) == 1.0);
        let sb_after_fast_ks = &dag.instrs_variances[lut1.0][LOW_PRECISION_PARTITION];
        assert!(
            sb_after_fast_ks.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION))
                == 1.0
        );
        ...
        for op_i in (lut1.0 + 1)..lut2.0 {
            ...
            assert!(sb.coeff(s2_inp(LOW_PRECISION_PARTITION)) == 0.0);
            assert!(sb.coeff(s2_inp(HIGH_PRECISION_PARTITION)) == 0.0);
            if sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION)) >= 1.0 {
                assert!(
                    sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION))
                        == sb.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION))
                );
            } else {
                assert!(sb.coeff(s2_pbs(LOW_PRECISION_PARTITION)) == 1.0);
                assert!(sb.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION)) == 0.0);
            }
        }
        let sb = &dag.instrs_variances[lut2.0][HIGH_PRECISION_PARTITION];
        assert!(sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION)) >= 1.0);
    }
    ...
    fn test_rounded_v3_classic_first_layer_second_layer() {
        ...
        assert!(
            dag.instrs_variances[free_input1.0][HIGH_PRECISION_PARTITION]
                .coeff(s2_inp(HIGH_PRECISION_PARTITION))
                == 1.0
        );
        // First layer tlu
        ...
        assert!(sb.coeff(s2_inp(LOW_PRECISION_PARTITION)) == 0.0);
        assert!(sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION)) == 1.0);
        assert!(sb.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION)) == 1.0);
        ...
        for op_i in (input1.0 + 1)..rounded1.0 {
            ...
            } = &dag.operators[op_i]
            {
                ...
                let input0_coeff_pbs_high = input0_sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION));
                let input0_coeff_pbs_low = input0_sb.coeff(s2_pbs(LOW_PRECISION_PARTITION));
                let input0_coeff_fks =
                    input0_sb.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION));
                if bit_extract {
                    ...
                    let input1_coeff_pbs_high = input1_sb.coeff(s2_pbs(HIGH_PRECISION_PARTITION));
                    let input1_coeff_pbs_low = input1_sb.coeff(s2_pbs(LOW_PRECISION_PARTITION));
                    let input1_coeff_fks =
                        input1_sb.coeff(s2_fks(HIGH_PRECISION_PARTITION, LOW_PRECISION_PARTITION));
                    if first_bit_erase {
                        assert!(input0_coeff_pbs_low == 0.0);
                    } else {
                        assert!(input0_coeff_pbs_low >= 1.0);
