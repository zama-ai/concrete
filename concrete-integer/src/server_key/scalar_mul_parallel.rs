...
impl ServerKey {
    ...
    pub fn unchecked_small_scalar_mul_assign_parallelized(
        ...
        ctxt.ct_vec.par_iter_mut().for_each(|ct_i| {
    ...
    /// assert_eq!(msg * scalar % modulus, clear);
    /// ```
    pub fn smart_small_scalar_mul_assign_parallelized(&self, ctxt: &mut Ciphertext, scalar: u64) {
        if !self.is_small_scalar_mul_possible(ctxt, scalar) {
            self.full_propagate_parallelized(ctxt);
        }
    ...
    /// assert_eq!(msg * scalar % modulus, clear);
    /// ```
    pub fn smart_scalar_mul_parallelized(&self, ct: &mut Ciphertext, scalar: u64) -> Ciphertext {
        let zero = self.create_trivial_zero(
            ct.ct_vec.len(),
            ct.message_modulus_vec.clone(),
            ct.key_id_vec.clone(),
        );
        if scalar == 0 || ct.ct_vec.is_empty() {
            return zero;
        }
        ...
        let terms = Mutex::new(Vec::<Ciphertext>::new());
    ...
    pub fn smart_scalar_mul_assign_parallelized(&self, ctxt: &mut Ciphertext, scalar: u64) {
