...
impl ServerKey {
    ...
    pub fn smart_binary_op_seq_parallelized<'this, 'item>(
        &'this self,
        ct_seq: impl IntoIterator<Item = &'item mut Ciphertext>,
        op: impl for<'a> Fn(&'a ServerKey, &'a mut Ciphertext, &'a mut Ciphertext) -> Ciphertext + Sync,
    ) -> Option<Ciphertext> {
        enum CiphertextCow<'a> {
            Borrowed(&'a mut Ciphertext),
            Owned(Ciphertext),
        }
        ...
        fn reduce_impl(
            ...
            op: &(dyn for<'a> Fn(&'a ServerKey, &'a mut Ciphertext, &'a mut Ciphertext) -> Ciphertext
                  + Sync),
        ) -> Option<Ciphertext> {
            use rayon::prelude::*;
            ...
            if ct_seq.is_empty() {
                ...
                while ct_seq.len() > 1 {
                    let results = Mutex::new(Vec::<Ciphertext>::with_capacity(ct_seq.len() / 2));
