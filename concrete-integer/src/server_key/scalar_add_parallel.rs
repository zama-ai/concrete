...
impl ServerKey {
    ...
    /// ```
    pub fn smart_scalar_add_parallelized(&self, ct: &mut Ciphertext, scalar: u64) -> Ciphertext {
    ...
    /// assert_eq!(msg + scalar, dec);
    /// ```
    pub fn smart_scalar_add_assign_parallelized(&self, ct: &mut Ciphertext, scalar: u64) {
        if !self.is_scalar_add_possible(ct, scalar) {
            self.full_propagate_parallelized(ct);
        }
