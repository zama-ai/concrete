...
impl ServerKey {
    ...
    pub fn unchecked_add_crt_assign_parallelized(
        ...
        ct_left
            .ct_vec
            .par_iter_mut()
            .zip(ct_right.ct_vec.par_iter())
            .for_each(|(block_left, block_right)| {
                self.key.unchecked_add_assign(block_left, block_right);
            });
    }
    ...
    where
        F: Fn(u64) -> u64,
    {
        let basis = ct1.message_modulus_vec.clone();
        for (i, block) in ct1.ct_vec.iter_mut().enumerate() {
            let acc = self.key.generate_accumulator(|x| f(x) % basis[i]);
            self.key
