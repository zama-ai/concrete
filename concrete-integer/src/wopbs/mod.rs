...
pub fn encode_mix_radix(val: u64, basis: &Vec<u64>) -> Vec<u64> {
    ...
    for basis in basis.iter() {
        ...
        rem = div - quo*basis;
        div = quo;
        // fill the vector with the message moduli5
        output.push(rem);
...
impl WopbsKey {
    ...
    pub fn wopbs_with_degree<T>(
        ...
        for block in ct_in.blocks_mut().iter_mut() {
            ...
            println!("IN WOPBS_WITH_DEGREE : nb_bit_to_extract = {}, delta = {}",
            ...
            F: Fn(u64) -> u64,
    {
        ...
        for i in 0..(1<< total_bit) {
            ...
            for (j, block) in ct.blocks.iter().enumerate() {
                ...
                let delta: u64 = (1 << 63) /
                    (self.wopbs_key.param.message_modulus.0 * self.wopbs_key.param
                        .carry_modulus.0) as u64;
                vec_lut[j][i as usize] = ((f((value % (1<<deg)) % block
                    .message_modulus.0 as u64)) % block.message_modulus.0 as u64) * delta;
                println!("Index = {}, value = {}, mod deg = {}, mod_msg = {}, deg = {}, num LUT =\
                 {}", i, value, (1<<deg), block.message_modulus.0 , deg, basis.len() - 1 - j );
                value >>= deg;
            }
        }
        vec_lut
    ...
    // /// assert_eq!(res, clear);
    // /// ```
    pub fn generate_lut_bivariate_radix<F>(&self, ct1: &RadixCiphertext, ct2: &RadixCiphertext, f:F) ->
                                                                                     Vec<Vec<u64>>
        where
            F: Fn(u64, u64) -> u64,
    {
        ...
        for lut_index_val in 0..(1<< total_bit) {
            ...
            println!("index = {}, split = {:?}, vec_deg_basis = {:?}, basis={}, \
            \
            decoded_val = \
            {:?}, \
            f_dec = {}, encoded = \
            {:?}", lut_index_val, split, vec_deg_basis, basis, decoded_val,
                     f_val, encoded_f_val);
        ...
        //     let mut val_ct1 = i % (1 << nb_bit_to_extract[0]);
        //     let mut val_ct2 = i >> nb_bit_to_extract[0];
        //     let mut tmp_ct1 = 0;
        //     let mut tmp_ct2 = 0;
        //
        //     for (j, block) in ct1.ct_vec.iter().enumerate() {
        //         let deg = f64::log2((block.degree.0 + 1) as f64).ceil() as u64;
        //         let nb_carry_bit = f64::log2((block.degree.0 + 1 - ct1.message_modulus_vec[0]) as f64).ceil() as u64;
        //         tmp_ct1 += (val_ct1 % ct1.message_modulus_vec[0]) << (ct1.message_modulus_vec[0]
        //             * j);
        //         let tmp_carry = (val_ct1 >> block_shift) & nb_carry_bit;
        //         val_ct1 = (val_ct1 >> deg) + tmp_carry;
        //     }
        //
        //     for (j, block) in ct2.ct_vec.iter().enumerate() {
        //         let deg = f64::log2((block.degree.0 + 1) as f64).ceil() as u64;
        //         let nb_carry_bit = f64::log2((block.degree.0 + 1 - ct1.message_modulus_vec[0]) as f64).ceil() as u64;
        //         tmp_ct2 += (val_ct2 % ct1.message_modulus_vec[0]) << (ct1.message_modulus_vec[0]
        //             * j);
        //         let tmp_carry = (val_ct2 >> block_shift) & nb_carry_bit;
        //         val_ct2 = (val_ct2 >> deg) + tmp_carry;
        //     }
        //
        //
        //     for (j, block) in ct2.ct_vec.iter().enumerate() {
