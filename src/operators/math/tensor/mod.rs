...
pub trait Tensor: Sized {
    fn named_print(slice: &[Self], s: std::string::String);
...
macro_rules! impl_trait_tensor {
    ($T:ty,$DOC:expr) => {
        impl Tensor for $T {
            ...
            /// ```
            fn named_print(slice: &[$T], s: std::string::String) {
                print!("[{}] tensor = [", s);
                for elt in slice {
                    print!("{}, ", elt);
                }
            ...
            fn get_normal_random_default(mean: f64, std_dev: f64) -> $T {
                // Random::rng_normal(mean, std_dev)
                Random::openssl_normal(mean, std_dev)
            }
            ...
            fn add(res: &mut [$T], t0: &[$T], t1: &[$T]) {
                debug_assert!(res.len() == t0.len(), "operators::math::tensor::add : res.len() = {} != t0.len() = {}", res.len(), t0.len()) ;
                debug_assert!(res.len() == t1.len(), "operators::math::tensor::add : res.len() = {} != t1.len() = {}", res.len(), t1.len()) ;
            ...
            fn add_inplace(res: &mut [$T], t: &[$T]) {
                debug_assert!(res.len() == t.len(), "operators::math::tensor::add_inplace : res.len() = {} != t.len() = {}", res.len(), t.len()) ;
            ...
            fn sub(res: &mut [$T], t0: &[$T], t1: &[$T]) {
                debug_assert!(res.len() == t0.len(), "operators::math::tensor::sub : res.len() = {} != t0.len() = {}", res.len(), t0.len()) ;
                debug_assert!(res.len() == t1.len(), "operators::math::tensor::sub : res.len() = {} != t1.len() = {}", res.len(), t1.len()) ;
                for ((res_ref, t0_val), t1_val) in res.iter_mut().zip(t0).zip(t1) {
            ...
            fn sub_inplace(res: &mut [$T], t: &[$T]) {
                debug_assert!(res.len() == t.len(), "operators::math::tensor::sub_inplace : res.len() = {} != t.len() = {}", res.len(), t.len()) ;
            ...
            fn scalar_mul(res: &mut [$T], t_in: &[$T], n: $T) {
                debug_assert!(res.len() == t_in.len(), "operators::math::tensor::scalar_mul : res.len() = {} != t_in.len() = {}", res.len(), t_in.len()) ;
            ...
            fn sub_scalar_mul(res: &mut [$T], t_in: &[$T], n: $T) {
                debug_assert!(res.len() == t_in.len(), "operators::math::tensor::sub_scalar_mul : res.len() = {} != t_in.len() = {}", res.len(), t_in.len()) ;
                for (res_ref, t_in_val) in res.iter_mut().zip(t_in) {
            ...
            fn add_scalar_mul(res: &mut [$T], t_in: &[$T], n: $T) {
                debug_assert!(res.len() == t_in.len(), "operators::math::tensor::add_scalar_mul : res.len() = {} != t_in.len() = {}", res.len(), t_in.len()) ;
                for (res_ref, t_in_val) in res.iter_mut().zip(t_in) {
            ...
            fn binary_multisum(t_torus: &[$T], t_bool: &[$T]) -> $T {
                debug_assert!(t_bool.len() * <$T as Types>::TORUS_BIT >= t_torus.len(), "operators::math::tensor::binary_multisum : the key is too short") ;
                let mut res: $T = 0;
            ...
            fn get_binary_multisum(t_torus: &[$T], t_bool: &[$T]) -> $T {
                debug_assert!(t_bool.len() * <$T as Types>::TORUS_BIT >= t_torus.len(), "operators::math::tensor::get_binary_multisum : the key is too short") ;
                let mut res: $T = 0;
                // loop over coefficients of a Torus vector
            ...
            fn round_to_closest_multiple(
                ...
                debug_assert!(t_res.len() == t_input.len(), "operators::math::tensor::round_to_closest_multiple : t_res.len() = {} =! t_input.len() = {}", t_res.len(), t_input.len()) ;
            ...
            fn int_to_float(t_res: &mut [f64], t_int: &[$T]) {
                debug_assert!(t_res.len() == t_int.len(), "operators::math::tensor::int_to_float : t_res.len() = {} =! t_int.len() = {}", t_res.len(), t_int.len()) ;
            ...
            fn float_to_int(t_res: &mut [$T], t_float: &[f64]) {
                debug_assert!(t_res.len() == t_float.len(), "operators::math::tensor::float_to_int : t_res.len() = {} =! t_float.len() = {}", t_res.len(), t_float.len()) ;
            ...
            fn compute_modular_distance(t_res: &mut [$T], t_x: &[$T], t_y: &[$T]) {
                debug_assert!(t_res.len() == t_x.len(), "operators::math::tensor::compute_modular_distance : t_res.len() = {} =! t_x.len() = {}", t_res.len(), t_x.len()) ;
                debug_assert!(t_res.len() == t_y.len(), "operators::math::tensor::compute_modular_distance : t_res.len() = {} =! t_y.len() = {}", t_res.len(), t_y.len()) ;
            ...
            fn compute_signed_modular_distance(t_res: &mut [f64], t_x: &[$T], t_y: &[$T]) {
                debug_assert!(t_res.len() == t_x.len(), "operators::math::tensor::compute_signed_modular_distance : t_res.len() = {} =! t_x.len() = {}", t_res.len(), t_x.len()) ;
                debug_assert!(t_res.len() == t_y.len(), "operators::math::tensor::compute_signed_modular_distance : t_res.len() = {} =! t_y.len() = {}", t_res.len(), t_y.len()) ;
                for (res, (x, y)) in t_res.iter_mut().zip(t_x.iter().zip(t_y.iter())) {
                    let d0 = x.wrapping_sub(*y);
                    let d1 = y.wrapping_sub(*x);
