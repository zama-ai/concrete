...
impl LWEBSK {
    ...
    pub fn get_polynomial_size_log(&self) -> usize {
        return f64::log2(self.polynomial_size as f64) as usize;
    }
    ...
    pub fn generate_functional_look_up_table<F: Fn(f64) -> f64>(
        &self,
        encoder_input: &pro_api::Encoder,
        encoder_output: &pro_api::Encoder,
        f: F,
    ) -> Result<Vec<Torus>, ProAPIError> {
        ...
        for i in 0..self.polynomial_size {
            ...
            // let decoded: f64 =
            //     crypto::Encoding::decode(encoded, encoder_input.o, encoder_input.delta);
            let decoded: f64 = encoder_input.decode_operators(encoded)?;
    ...
    pub fn generate_identity_look_up_table(
        ...
        return self.generate_functional_look_up_table(encoder_input, encoder_output, |x| x);
    }
    ...
    /// * an LWEBSK
    pub fn new(
        sk_input: &pro_api::LWESecretKey,
        sk_output: &pro_api::RLWESecretKey,
        base_log: usize,
        level: usize,
    ) -> LWEBSK {
    ...
    pub fn write_in_file_bytes(&self, path: &str) {
        ...
        tensor[0] = unsafe { transmute::<f64, u64>(self.variance) };
        ...
        {
            couple[0] = unsafe { transmute::<f64, u64>(c.re) };
            couple[1] = unsafe { transmute::<f64, u64>(c.im) };
        }
        Tensor::write_in_file(&tensor, path).unwrap();
    }

    pub fn read_in_file_bytes(path: &str) -> pro_api::LWEBSK {
        ...
        let mut res = pro_api::LWEBSK {
            variance: unsafe { transmute::<u64, f64>(tensor_1[0]) },
        ...
        for (couple, c) in tensor_2.chunks(2).zip(res.ciphertexts.iter_mut()) {
            *c = CTorus::new(unsafe { transmute::<u64, f64>(couple[0]) }, unsafe {
                transmute::<u64, f64>(couple[1])
            });
        }
        res
    }
