...
//     // encode and decode
//     let mut plaintext = pro_api::Plaintext::new_single_encoder(messages.len(), &encoder);
...
fn test_zero_x_set_encoders_x_encode_x_decode() {
    ...
    let mut encoders: Vec<pro_api::Encoder> = vec![pro_api::Encoder::zero(); nb_messages];
    ...
    for (m, e) in izip!(messages.iter_mut(), encoders.iter_mut()) {
        ...
        *m = random_message!(min, max);
        ...
        // create an encoder
        e.copy(&pro_api::Encoder::new(min, max, precision, padding).unwrap());
        println!("encoder {}", e);
    }

    // encode and decode
    let mut plaintext = pro_api::Plaintext::zero(nb_messages);
...
fn test_zero_x_set_encoders_from_one_x_encode_x_decode() {
    ...
    // encode and decode
    let mut plaintext = pro_api::Plaintext::zero(messages.len());
...
fn test_zero_x_set_nth_encoder_from_one_x_encode_x_decode() {
    ...
    let (min, max) = generate_random_interval!();
    let (precision, padding) = generate_precision_padding!(8, 8);
    let encoder_bis = pro_api::Encoder::new(min, max, precision, padding).unwrap();
