use crate::operators::math::{Random, Tensor};
use crate::pro_api;
...
fn test_new_centered_x_encode_single_x_decode_single() {
    ...
    // create an encoder
    let encoder = pro_api::Encoder::new_centered(
        min + (max - min) / 2.,
        (max - min) / 2.,
        precision,
        padding,
...
fn test_zero_x_is_valid() {
    ...
    let encoder = pro_api::Encoder::zero();
...
fn test_new_x_encode_single_x_copy_x_decode_single() {
    ...
    let (min, max) = generate_random_interval!();
    let (precision, padding) = generate_precision_padding!(8, 8);
    let mut encoder_copy = pro_api::Encoder::new(min, max, precision, padding).unwrap();
...
fn margins_with_integers() {
    ...
    // create an encoder
    let encoder = pro_api::Encoder::new(min, max, power, padding).unwrap();
