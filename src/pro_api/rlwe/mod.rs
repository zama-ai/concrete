...
use crate::npe;
use crate::operators::{crypto, math};
use crate::pro_api;
...
impl RLWE {
    /// Instantiate a new RLWE filled with zeros from a polynomial size, a dimension and a number of ciphertexts
    ///
    /// # Arguments
    ...
    ///
    /// # Output
    /// * a new instantiation of an RLWE
    ...
    /// let empty_ciphertexts = RLWE::zero(1024, 1, 3).unwrap();
    /// ```
    pub fn zero(
        polynomial_size: usize,
        dimension: usize,
        nb_ciphertexts: usize,
    ) -> Result<pro_api::RLWE, ProAPIError> {
        if nb_ciphertexts == 0 {
        ...
        Ok(RLWE {
            ciphertexts: vec![0; (dimension + 1) * polynomial_size * nb_ciphertexts],
            variances: vec![0.; polynomial_size * nb_ciphertexts],
            dimension: dimension,
            polynomial_size: polynomial_size,
            nb_ciphertexts: nb_ciphertexts,
            encoders: vec![pro_api::Encoder::zero(); nb_ciphertexts * polynomial_size],
        })
    }
    ...
    ///
    /// # Output
    /// * a new instantiation of an RLWE encrypting the plaintexts provided in one ciphertext RLWE
    ///
    /// # Example
    /// ```rust
    /// use concrete_lib::*;
    ...
    /// let sk = pro_api::RLWESecretKey::new(&RLWE128_1024_1);
    ///
    ...
    /// ```
    pub fn new_encrypt_1_ciphertext(
        sk: &pro_api::RLWESecretKey,
        plaintexts: &pro_api::Plaintext,
    ) -> Result<RLWE, ProAPIError> {
        // allocate the RLWE structure
        let mut res = RLWE::zero(sk.polynomial_size, sk.dimension, 1)?;

        // create a vec with the good size for the raw plaintexts
        let mut tmp_pt: Vec<Torus> = vec![0; sk.polynomial_size];
    ...
    ///
    /// # Output
    /// * a new instantiation of an RLWE encrypting the plaintexts provided in one RLWE ciphertext
    ///
    /// # Example
    /// ```rust
    /// use concrete_lib::*;
    ...
    /// ```
    pub fn new_encode_encrypt_1_ciphertext(
        sk: &pro_api::RLWESecretKey,
        messages: &[f64],
    ...
    ///
    /// # Output
    /// * a new instantiation of an RLWE encrypting the plaintexts provided in as many RLWE ciphertexts as there was messages
    ///
    /// # Example
    /// ```rust
    /// use concrete_lib::*;
    ...
    pub fn new_encrypt_on_cst(
        ...
        // allocate the RLWE structure
        let mut res = RLWE::zero(sk.polynomial_size, sk.dimension, nb_ciphertexts)?;
    ...
    /// ```
    pub fn new_encode_encrypt_on_cst(
        sk: &pro_api::RLWESecretKey,
        messages: &[f64],
        encoder: &pro_api::Encoder,
    ) -> Result<RLWE, ProAPIError> {
    ...
    pub fn encrypt_raw(
        ...
        else if sk.std_dev < f64::powi(2., -(<Torus as Types>::TORUS_BIT as i32) + 2) {
    ...
    /// // encrypt and decrypt
    /// let ct = pro_api::RLWE::new_encrypt_on_cst(&sk, &enc_messages).unwrap();
    /// let res = ct.decrypt(&sk).unwrap();
    /// ```
    pub fn decrypt(&self, sk: &pro_api::RLWESecretKey) -> Result<Vec<f64>, ProAPIError> {
    ...
    pub fn decrypt_with_encoders(
        &self,
        sk: &pro_api::RLWESecretKey,
    ) -> Result<(Vec<f64>, Vec<pro_api::Encoder>), ProAPIError> {
        let messages = self.decrypt(sk)?;
        let mut encoders: Vec<pro_api::Encoder> = vec![pro_api::Encoder::zero(); messages.len()];
        let mut cpt: usize = 0;
    ...
    pub fn extract_1_lwe(
        ...
        n_coeff: usize,
        n_ciphertext: usize,
    ) -> Result<pro_api::LWE, ProAPIError> {
        ...
        let mut res = pro_api::LWE::zero(lwe_dimension, 1)?;
        ...
        // fill the body
        res.ciphertexts[lwe_dimension] = self.ciphertexts[index_body];
        ...
        for polynomial in res.ciphertexts.chunks_mut(self.polynomial_size) {
            if polynomial.len() == self.polynomial_size {
                polynomial.reverse();
                for elt in polynomial.get_mut(0..rot).unwrap().iter_mut() {
                    *elt = elt.wrapping_neg();
                }
                polynomial.rotate_left(rot);
            }
        }
    ...
    /// Add small messages to a RLWE ciphertext and does not change the encoding but changes the bodies of the ciphertexts
    ...
    pub fn add_constant_static_encoder_inplace(
        ...
        for (ciphertext, encoders) in izip!(
            self.ciphertexts
                .chunks_mut((self.dimension + 1) * self.polynomial_size),
    ...
    /// Add messages to an RLWE ciphertext and translate the interval of a distance equal to the message but does not change either the bodies or the masks of the ciphertexts
    /// the first message is added to the first coefficient that has a valid encoder
    /// the second message is added to the second coefficient that has a valid encoder
    /// ...
    ///
    ...
    /// Compute an homomorphic addition between two RLWE ciphertexts and the center of the output Encoder is the sum of the two centers of the input Encoders
    ///
    /// # Arguments
    /// * `ct` - an LWE struct
    ///
    ...
    pub fn add_centered(&self, ct: &pro_api::RLWE) -> Result<pro_api::RLWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// * PolynomialSizeError - if the ciphertexts have incompatible polynomial size
    /// * DeltaError - if the ciphertext encoders have incompatible deltas
    pub fn add_centered_inplace(&mut self, ct: &pro_api::RLWE) -> Result<(), ProAPIError> {
        ...
        for (ciphertext, encoders, encoders_ct, self_variances, ct_variances) in izip!(
            self.ciphertexts
                .chunks_mut(self.polynomial_size * (self.dimension + 1)),
            self.encoders.chunks_mut(self.polynomial_size),
    ...
    pub fn add_with_padding(&self, ct: &pro_api::RLWE) -> Result<pro_api::RLWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// * NotEnoughPaddingError - if there is no padding
    /// * DeltaError - if the ciphertexts have incompatile deltas
    pub fn add_with_padding_inplace(&mut self, ct: &pro_api::RLWE) -> Result<(), ProAPIError> {
    ...
    pub fn sub_with_padding(&self, ct: &pro_api::RLWE) -> Result<pro_api::RLWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// * NotEnoughPaddingError - if there is no padding
    /// * DeltaError - if the ciphertexts have incompatile deltas
    pub fn sub_with_padding_inplace(&mut self, ct: &pro_api::RLWE) -> Result<(), ProAPIError> {
        // check the dimensions
        if self.dimension != ct.dimension {
            return Err(DimensionError!(self.dimension, ct.dimension));
        }
        ...
        for (ciphertext, enc1_list, enc2_list) in izip!(
            self.ciphertexts
                .chunks_mut((self.dimension + 1) * self.polynomial_size),
            ...
            for (coeff, enc1, enc2) in izip!(
                ...
                if enc1.is_valid() && enc2.is_valid() {
                    let correction: Torus =
                        1 << (<Torus as Types>::TORUS_BIT - enc1.nb_bit_padding);
    ...
    /// Multiply RLWE ciphertexts with small integer messages and does not change the encoding but changes the bodies and masks of the ciphertexts
    /// # Argument
    ...
    pub fn mul_constant_static_encoder_inplace(
        ...
        for (ciphertext, m, encoder_list, variance_list) in izip!(
            self.ciphertexts
                .chunks_mut((self.dimension + 1) * self.polynomial_size),
            messages.iter(),
            self.encoders.chunks_mut(self.polynomial_size),
    ...
    /// Multiply each RLWE ciphertext with a real constant and do change the encoding and the ciphertexts by consuming some bits of padding
    ...
    pub fn mul_constant_with_padding_inplace(
        ...
        for (ciphertext, c, encoders, variances) in izip!(
            ...
            for (b, encoder, var) in izip!(
                ...
                *var = npe::LWE::single_scalar_mul(*var, scal);
                ...
                if encoder.is_valid() {
                    ...
                    let tmp_encoder = pro_api::Encoder::new(
                        new_o,
                        new_max,
    ...
    pub fn get_ciphertext_size(&self) -> usize {
        return self.polynomial_size * (self.dimension + 1);
    }
    ...
    pub fn load(path: &str) -> Result<RLWE, Box<dyn Error>> {
        read_from_file(path)
    }
}
...
// pub nb_ciphertexts: usize,
// pub encoders: Vec<pro_api::Encoder>,
/// Print needed pieces of information about an RLWE
impl fmt::Display for RLWE {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let n = 2;
        let mut to_be_print: String = "".to_string();
        to_be_print = to_be_print + " RLWE {\n         -> samples = [";
        ...
        to_be_print = to_be_print + "         -> variances = [";
