...
impl LWE {
    /// Instantiate a new LWE filled with zeros from a dimension and a number of ciphertexts
    ...
    /// let empty_ciphertexts = LWE::zero(630, 5).unwrap();
    /// ```
    pub fn zero(dimension: usize, nb_ciphertexts: usize) -> Result<pro_api::LWE, ProAPIError> {
        if nb_ciphertexts == 0 {
        ...
        Ok(LWE {
            ...
            encoders: vec![pro_api::Encoder::zero(); nb_ciphertexts],
        })
    }

    /// Copy one ciphertext from an LWE structure inside the self LWE structure
    ...
    /// * `ct` - the LWE structure we will copy a ciphertext from
    ...
    /// extract the n-th of the LWE ciphertexts from an LWE structure and output a new LWE structure with only a copy of this ciphertext
    ///
    /// # Arguments
    ...
    /// let ct = LWE::zero(630, 6).unwrap();
    ///
    /// // extract the first ciphertext of ct
    /// let ct_extracted = ct.extract_nth(0).unwrap();
    /// ```
    pub fn extract_nth(&self, n: usize) -> Result<pro_api::LWE, ProAPIError> {
    ...
    /// let ct = LWE::new_encrypt(&sk, &pt);
    /// ```
    pub fn new_encrypt(
        sk: &pro_api::LWESecretKey,
        plaintexts: &pro_api::Plaintext,
    ) -> Result<pro_api::LWE, ProAPIError> {
        let mut res = LWE::zero(sk.dimension, plaintexts.nb_plaintexts)?;
        res.encrypt(sk, plaintexts)?;
        Ok(res)
    }

    /// Encode messages and then directly encrypt the plaintexts into an LWE structure
    ///
    /// # Arguments
    ...
    /// ```
    pub fn new_encode_encrypt(
        sk: &pro_api::LWESecretKey,
        messages: &[f64],
        encoder: &pro_api::Encoder,
    ) -> Result<LWE, ProAPIError> {
        ...
        let mut result_encoder: pro_api::Encoder = encoder.clone();
    ...
    ///
    /// // create a new LWE that encrypts pt
    /// let mut ct = LWE::zero(sk.dimension, messages.len()).unwrap();
    /// ct.encrypt(&sk, &pt).unwrap();
    /// ```
    pub fn encrypt(
        &mut self,
        sk: &pro_api::LWESecretKey,
        ...
        for (output_enc, input_enc) in izip!(self.encoders.iter_mut(), plaintexts.encoders.iter()) {
            // copy the Encoders from the Plaintexts to the LWE
    ...
    /// let encoder = Encoder::new(-5., 5., 8, 0).unwrap();
    ///
    /// // create a list of encoded messages
    ...
    ///
    /// // create a new LWE that encrypts pt
    /// let mut ct = LWE::zero(sk.dimension, pt.len()).unwrap();
    ...
    /// ```
    pub fn decrypt(&self, sk: &pro_api::LWESecretKey) -> Result<Vec<f64>, ProAPIError> {
    ...
    /// Add messages to a LWE ciphertext and translate the interval of a distance equal to the message but does not change either the bodies or the masks of the ciphertexts
    ///
    /// # Argument
    /// * `messages` - a list of messages as f64
    ///
    ...
    ///
    /// // encode and encrypt
    /// let ciphertext_1 = LWE::new_encode_encrypt(&secret_key, &messages_1, &encoder_1).unwrap();
    ...
    /// ```
    pub fn add_centered(&self, ct: &pro_api::LWE) -> Result<pro_api::LWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn add_centered_inplace(&mut self, ct: &pro_api::LWE) -> Result<(), ProAPIError> {
    ...
    /// ```
    pub fn add_with_padding(&self, ct: &pro_api::LWE) -> Result<pro_api::LWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn add_with_padding_inplace(&mut self, ct: &pro_api::LWE) -> Result<(), ProAPIError> {
    ...
    /// ```
    pub fn sub_with_padding(&self, ct: &pro_api::LWE) -> Result<pro_api::LWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn sub_with_padding_inplace(&mut self, ct: &pro_api::LWE) -> Result<(), ProAPIError> {
        for (self_enc, ct_enc) in izip!(self.encoders.iter(), ct.encoders.iter()) {
            // check same paddings
            if self_enc.nb_bit_padding != ct_enc.nb_bit_padding {
                return Err(PaddingError!(
    ...
    /// Multiply LWE ciphertexts with small integer messages and does not change the encoding but changes the bodies and masks of the ciphertexts
    ///
    /// # Argument
    ...
    ///
    /// // encode and encrypt
    /// let ciphertext = LWE::new_encode_encrypt(&secret_key, &messages_1, &encoder).unwrap();
    ...
    /// ```
    pub fn opposite_nth(&self, n: usize) -> Result<pro_api::LWE, ProAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn opposite_nth_inplace(&mut self, n: usize) -> Result<(), ProAPIError> {
        // check the index n
        if n >= self.nb_ciphertexts {
            return Err(IndexError!(self.nb_ciphertexts, n));
        }
    ...
    /// ```
    pub fn keyswitch(&self, ksk: &pro_api::LWEKSK) -> Result<pro_api::LWE, ProAPIError> {
        // allocation for the result
        let mut res: pro_api::LWE = pro_api::LWE::zero(ksk.dimension_after, self.nb_ciphertexts)?;
    ...
    /// Compute a bootstrap on the n-th LWE from the self LWE structure
    ///
    /// # Argument
    ...
    /// let secret_key_before = pro_api::LWESecretKey::new(&pro_api::LWE128_630);
    /// let secret_key_after = rlwe_secret_key.to_lwe_secret_key();
    ///
    /// // bootstrapping key
    /// let bootstrapping_key =
    ///     pro_api::LWEBSK::new(&secret_key_before, &rlwe_secret_key, base_log, level);
    ///
    /// // a list of messages that we encrypt
    /// let ciphertext_before =
    ///     pro_api::LWE::new_encode_encrypt(&secret_key_before, &messages, &encoder).unwrap();
    ...
    pub fn bootstrap_nth(
        ...
        return self.bootstrap_nth_with_function(bsk, |x| x, &self.encoders[n], n);
    }

    /// Compute a bootstrap and apply an arbitrary function to the given LWE ciphertext
    ///
    /// # Argument
    /// * `bsk` - the bootstrapping key
    ...
    /// // a list of messages that we encrypt
    /// let ciphertext_before =
    ///     pro_api::LWE::new_encode_encrypt(&secret_key_before, &messages, &encoder_input).unwrap();
    ///
    /// let ciphertext_out = ciphertext_before
    ...
    pub fn bootstrap_nth_with_function<F: Fn(f64) -> f64>(
        ...
        bsk: &pro_api::LWEBSK,
        f: F,
        encoder_output: &pro_api::Encoder,
        n: usize,
    ) -> Result<pro_api::LWE, ProAPIError> {
        // check the index n
        if n >= self.nb_ciphertexts {
        ...
        let mut new_encoder_output: pro_api::Encoder = encoder_output.clone();
        ...
        if nb_bit_overlap > 0 {
            println!(
                "{}: {} bit(s) of precision lost over {} bit(s) of message originally. Consider increasing the number of level and/or the log base parameters.",
    ...
    /// // a list of messages that we encrypt
    /// let ciphertext_1 =
    ///     pro_api::LWE::new_encode_encrypt(&secret_key_before, &messages_1, &encoder_1).unwrap();
    ///
    /// let ciphertext_2 =
    ///     pro_api::LWE::new_encode_encrypt(&secret_key_before, &messages_2, &encoder_2).unwrap();
    ///
    /// let ciphertext_out = ciphertext_1
    ///     .mul_with_bootstrap_nth(ciphertext_2, &bootstrapping_key, 2, 3)
    ///     .unwrap();
    /// ```
    pub fn mul_with_bootstrap_nth(
        &self,
        ct: pro_api::LWE,
        bsk: &pro_api::LWEBSK,
        n_self: usize,
