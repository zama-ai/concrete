...
impl Encoder {
    ...
    /// ```
    pub fn encode_single(&self, message: f64) -> Result<crypto_api::Plaintext, CryptoAPIError> {
    ...
    /// ```
    pub fn encode(&self, messages: &[f64]) -> Result<crypto_api::Plaintext, CryptoAPIError> {
        let mut result = crypto_api::Plaintext {
    ...
    /// ```
    pub fn copy(&mut self, encoder: &crypto_api::Encoder) {
        self.o = encoder.o;
    ...
    pub fn new_square_divided_by_four(
        &self,
        nb_bit_padding: usize,
    ) -> Result<crypto_api::Encoder, CryptoAPIError> {
        // valid encoder
        if !self.is_valid() {
