...
fn test_encode_encrypt_x_copy_in_nth_nth_inplace_x_decrypt() {
    ...
    let encoder_1 = crypto_api::Encoder::new(min1, max1, precision1, padding1).unwrap();
    ...
    let encoder_2 = crypto_api::Encoder::new(min2, max2, precision2, padding2).unwrap();
    ...
    let params = crypto_api::LWEParams::new(dimension, log_std_dev);
    let sk = crypto_api::LWESecretKey::new(&params);

    // encode and encrypt
    let mut ct1 = crypto_api::VectorLWE::encode_encrypt(&sk, &messages_1, &encoder_1).unwrap();
    let ct2 = crypto_api::VectorLWE::encode_encrypt(&sk, &messages_2, &encoder_2).unwrap();
...
fn test_encrypt_x_extract_nth_x_decrypt() {
    ...
    let ct = crypto_api::VectorLWE::encrypt(&sk, &pt).unwrap();
...
fn test_encrypt_x_decrypt() {
    ...
    // encode and encrypt
    let plaintext = crypto_api::Plaintext::encode(&messages, &encoder).unwrap();
...
fn test_encrypt_x_add_constant_static_encoder_inplace_x_decrypt() {
    ...
    // encode and encrypt
    let plaintext_1 = crypto_api::Plaintext::encode(&messages_1, &encoder).unwrap();
    let mut ciphertext = crypto_api::VectorLWE::encrypt(&secret_key, &plaintext_1).unwrap();
...
fn test_encode_encrypt_x_add_centered_inplace_x_decrypt() {
    ...
    // encode and encrypt
    let mut ciphertext1 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key, &messages1, &encoder1).unwrap();
    let ciphertext2 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key, &messages2, &encoder2).unwrap();
...
fn test_encode_encrypt_x_mul_constant_static_encoder_inplace_x_decrypt() {
    ...
    // encode and encrypt
    let mut ciphertext =
        crypto_api::VectorLWE::encode_encrypt(&secret_key, &messages1, &encoder).unwrap();
...
fn test_encode_encrypt_x_bootstrap_nth_x_decrypt() {
    ...
    for _i in 0..1 {
        ...
        let ciphertext_input =
            crypto_api::VectorLWE::encode_encrypt(&secret_key_input, &message, &encoder_input)
                .unwrap();
...
fn test_encode_encrypt_x_mul_from_bootstrap_nth_nth_x_decrypt() {
    ...
    // encode and encrypt
    let ciphertext_1 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key_input, &messages_1, &encoder_1).unwrap();
    let ciphertext_2 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key_input, &messages_2, &encoder_2).unwrap();
...
fn test_encode_encrypt_x_add_with_new_min_inplace_x_decrypt() {
    ...
    let mut ciphertext_1 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key, &messages_1, &encoder_1).unwrap();
    let ciphertext_2 =
        crypto_api::VectorLWE::encode_encrypt(&secret_key, &messages_2, &encoder_2).unwrap();
