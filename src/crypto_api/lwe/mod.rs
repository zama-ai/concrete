...
pub struct LWE {
    pub ciphertext: Vec<Torus>,
    pub variance: f64,
    pub dimension: usize,
    pub encoder: crypto_api::Encoder,
}
...
impl LWE {
    ...
    /// ```
    pub fn zero(dimension: usize) -> Result<crypto_api::LWE, CryptoAPIError> {
        Ok(LWE {
            ciphertext: vec![0; dimension + 1],
            variance: 0.,
            dimension: dimension,
            encoder: crypto_api::Encoder::zero(),
        })
    }
    ...
    pub fn encode_encrypt(
        ...
        let mut res = LWE {
            ciphertext: vec![0; sk.dimension + 1],
            variance: 0.,
    ...
    /// ```
    pub fn decrypt_decode(&self, sk: &crypto_api::LWESecretKey) -> Result<f64, CryptoAPIError> {
        // check dimensions
    ...
    pub fn add_constant_static_encoder(
        &self,
        message: f64,
    ) -> Result<crypto_api::LWE, CryptoAPIError> {
    ...
    pub fn add_constant_static_encoder_inplace(
        ...
        ec_tmp.o = 0.;
        self.ciphertext[self.dimension] = self.ciphertext[self.dimension]
    ...
    pub fn add_with_new_min_inplace(
        ...
        math::Tensor::add_inplace(&mut self.ciphertext, &ct.ciphertext);
    ...
    /// ```
    pub fn add_centered(&self, ct: &crypto_api::LWE) -> Result<crypto_api::LWE, CryptoAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn add_centered_inplace(&mut self, ct: &crypto_api::LWE) -> Result<(), CryptoAPIError> {
    ...
    /// ```
    pub fn add_with_padding_inplace(&mut self, ct: &crypto_api::LWE) -> Result<(), CryptoAPIError> {
        // check same paddings
    ...
    /// ```
    pub fn sub_with_padding_inplace(&mut self, ct: &crypto_api::LWE) -> Result<(), CryptoAPIError> {
        // check same paddings
        if self.encoder.nb_bit_padding != ct.encoder.nb_bit_padding {
            return Err(PaddingError!(
                self.encoder.nb_bit_padding,
        ...
        math::Tensor::sub_inplace(&mut self.ciphertext, &ct.ciphertext);

        // correction related to the subtraction
        let correction: Torus = 1 << (<Torus as Types>::TORUS_BIT - self.encoder.nb_bit_padding);
        self.ciphertext[self.dimension] = self.ciphertext[self.dimension].wrapping_add(correction);
    ...
    pub fn mul_constant_static_encoder_inplace(
        ...
        // multiplication
        math::Tensor::scalar_mul_inplace(&mut self.ciphertext, message as Torus);

        // apply correction
        self.ciphertext[self.dimension] = self.ciphertext[self.dimension].wrapping_sub(cor);
    ...
    pub fn mul_constant_with_padding_inplace(
        ...
        math::Tensor::scalar_mul_inplace(&mut self.ciphertext, scal);
        ...
        if negative {
            ...
            math::Tensor::neg_inplace(&mut self.ciphertext);

            // add correction if there is some padding
            let mut cor: Torus = 0;
            if self.encoder.nb_bit_padding > 0 {
                cor = (1 << (<Torus as Types>::TORUS_BIT - self.encoder.nb_bit_padding))
            ...
            self.ciphertext[self.dimension] = self.ciphertext[self.dimension].wrapping_add(cor);
    ...
    /// ```
    pub fn opposite(&self) -> Result<crypto_api::LWE, CryptoAPIError> {
        let mut res = self.clone();
    ...
    /// ```
    pub fn keyswitch(&self, ksk: &crypto_api::LWEKSK) -> Result<crypto_api::LWE, CryptoAPIError> {
        // allocation for the result
        let mut res: crypto_api::LWE = crypto_api::LWE::zero(ksk.dimension_after)?;
    ...
    ///     .unwrap();
    /// ```
    pub fn bootstrap(&self, bsk: &crypto_api::LWEBSK) -> Result<crypto_api::LWE, CryptoAPIError> {
    ...
    /// // encode and encrypt
    /// let ciphertext_before =
    ///     crypto_api::LWE::encode_encrypt(&secret_key_before, message, &encoder_input).unwrap();
    ///
    /// let ciphertext_out = ciphertext_before
    ...
    /// // a list of messages that we encrypt
    /// let ciphertext_1 =
    ///     crypto_api::LWE::encode_encrypt(&secret_key_before, message_1, &encoder_1).unwrap();
    ///
    /// let ciphertext_2 =
    ///     crypto_api::LWE::encode_encrypt(&secret_key_before, message_2, &encoder_2).unwrap();
    ///
    ...
    pub fn remove_padding_inplace(&mut self, nb: usize) -> Result<(), CryptoAPIError> {
        ...
        math::Tensor::bit_shift_left_inplace(&mut self.ciphertext, nb);
...
impl fmt::Display for LWE {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if self.ciphertext.len() <= 2 * n {
            for elt in self.ciphertext.iter() {
                to_be_print = to_be_print + &format!("{}, ", elt);
            }
            to_be_print += "]\n";
        } else {
            for elt in self.ciphertext[0..n].iter() {
                to_be_print = to_be_print + &format!("{}, ", elt);
            }
            to_be_print += "...";

            for elt in self.ciphertext[self.ciphertext.len() - n..].iter() {
                to_be_print = to_be_print + &format!("{}, ", elt);
            }
            to_be_print += "]\n";
        }
