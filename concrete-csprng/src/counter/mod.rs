...
impl<G: AesBatchedGenerator> AesCtrGenerator<G> {
    ...
    /// Tries to perform an unbounded fork of the current generator into `n_child` generators
    /// each able to yield as many random bytes as wanted.
    pub fn try_unbounded_fork(
        &mut self,
        n_child: ChildCount,
    ) -> Option<impl Iterator<Item = AesCtrGenerator<G>>> {
        self.try_bounded_fork(n_child, BytesPerChild(16))
            .map(|iter| iter.map(Self::bound_to_unbound))
    }
    ...
    where
        G: Send + Sync,
    {
        self.par_try_bounded_fork(n_child, BytesPerChild(16))
            .map(|par_iter| par_iter.map(Self::bound_to_unbound))
    }
    ...
    fn bound_to_unbound(mut gen: Self) -> Self {
        let mut state_bytes = [0u8; 16];
        state_bytes
            .iter_mut()
            .for_each(|b| *b = gen.generate_next());
        let state = State::from_aes_counter(AesCtr(u128::from_ne_bytes(state_bytes)));
        let mut generator = gen.generator;
        let batch = generator.generate_batch(state.aes_ctr);
        AesCtrGenerator {
            generator,
            state,
