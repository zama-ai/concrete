...
__host__ void scratch_cuda_integer_mult_radix_ciphertext_kb(
  ...
  switch (pbs_type) {
  case MULTI_BIT:
    max_buffer_size = get_max_buffer_size_multibit_bootstrap(
...
__host__ void host_integer_mult_radix_kb(
  ...
  //  Torus *host_pbs_out = (Torus *)malloc((polynomial_size + 1) * num_blocks *
  //                                        num_blocks * sizeof(Torus));
  //  std::ifstream infile("input.txt");
  //  std::string line;
  //  std::getline(infile, line);
  //  std::getline(infile, line);
  //  for (int lwe_id = 0; lwe_id < num_blocks * num_blocks; lwe_id++) {
  //    std::getline(infile, line);
  ...
  //      break;
  //    }
  //    auto cur_dst = &host_pbs_out[dst_id * (polynomial_size + 1)];
  //    for (int i = 0; i < polynomial_size + 1; i++) {
  //      iss >> cur_dst[i];
  //    }
  //  }
  //
  //  cuda_memcpy_async_to_gpu(lwe_pbs_out_array, host_pbs_out, (polynomial_size
  //  + 1) * num_blocks *
  //                                                                num_blocks *
  //                                                                sizeof(Torus),
  //                           stream, 0);
  ...
  //  print_debug("lsb_msb_after_pbs: ", lwe_pbs_out_array, 4 * 2048);
