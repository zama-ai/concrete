...
__host__ __device__ uint64_t get_buffer_size_wop_pbs(
    uint32_t lwe_dimension, uint32_t number_of_bits_to_extract[],
    uint32_t number_of_inputs) {

  int total_bits_to_extract = 0;
  for (int i = 0; i < number_of_inputs; i++) {
    total_bits_to_extract += number_of_bits_to_extract[i];
  }
  uint64_t buffer_size = (lwe_dimension + 1) * total_bits_to_extract *
...
__host__ void scratch_wop_pbs(
  ...
  uint32_t mbr_size = std::min(
      params::log2_degree, (int)total_bits_to_extract);
  if (allocate_gpu_memory) {
    uint64_t buffer_size =
        bit_extract_buffer_size +
        get_buffer_size_wop_pbs<Torus>(
            lwe_dimension, number_of_bits_to_extract_array, number_of_inputs) +
        get_buffer_size_cbs_vp<Torus>(glwe_dimension, polynomial_size,
                                      level_count_cbs, number_of_inputs,
...
__host__ void host_wop_pbs(
  ...
  host_circuit_bootstrap_vertical_packing<Torus, STorus, params>(
      ...
      level_count_cbs, total_bits_to_extract, number_of_inputs,
      max_shared_memory);
  check_cuda_error(cudaGetLastError());
}
#endif // WOP_PBS_H
