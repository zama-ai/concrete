  ...
  void SetUp(const ::benchmark::State &state) {
    ...
    for (int device = 0; device < num_gpus; device++) {
      ...
      integer_multiplication_setup(
          stream, &csprng, &lwe_sk_in, &lwe_sk_out, &d_bsk_array_per_gpu,
          &d_ksk_array_per_gpu, &plaintexts_1, &plaintexts_2,
          &d_lwe_ct_in_array_1_per_gpu, &d_lwe_ct_in_array_2_per_gpu,
          &d_lwe_ct_out_array_per_gpu, mem_ptr_per_gpu, lwe_dimension,
          glwe_dimension, polynomial_size, lwe_modular_variance,
          glwe_modular_variance, pbs_base_log, pbs_level, ksk_base_log,
          ksk_level, total_message_bits, number_of_blocks, message_modulus,
          carry_modulus, &delta, 1, 1, pbs_type, device);
...
(benchmark::State &st) {
  ...
  // uint64_t buffer_size = get_buffer_size_integer_mult_kb_u64(
...
static void IntegerMultiplicationBenchmarkGenerateParams(
  ...
  for (int operations_per_gpu = 1; operations_per_gpu < 10;
       operations_per_gpu++)
    for (auto x : params) {
