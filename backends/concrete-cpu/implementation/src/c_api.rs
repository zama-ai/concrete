...
mod utils {
    ...
    pub unsafe fn safe_serialize<T: Serialize + Versionize + Named>(
        ...
        println!("safe_serialize 1");
        let writer = core::slice::from_raw_parts_mut(buffer, buffer_len);
        println!("safe_serialize 2");
        // BANG!
        let serialized_size: usize = match bincode::serialized_size(&value) {
            Ok(size) => {
                let size = size + 201;
                if size > buffer_len as u64 {
                    println!("safe_serialize 3");
                    return 0;
                }
                // safe_serialize buf_len=263088 len=131392
                println!("safe_serialize!!! buf_len={} len={}", buffer_len, size);
                size as usize
            }
            Err(e) => {
                println!("safe_serialize 3 {}", e);
                return 0
            },
        };
    ...
    pub unsafe fn safe_deserialize<T: DeserializeOwned + Unversionize + Named>(
        ...
        println!("safe_deserialize len={}",buffer_len);
        let reader = core::slice::from_raw_parts(buffer, buffer_len);
        tfhe::safe_serialization::safe_deserialize(reader, SERIALIZE_SIZE_LIMIT).unwrap()
    }
    ...
    // TODO: Remove me when safe_serialization by thfe-rs is implemented for all object.
    pub unsafe fn unsafe_deserialize<T: DeserializeOwned>(buffer: *const u8, buffer_len: usize) -> T {
        let reader = core::slice::from_raw_parts(buffer, buffer_len);
